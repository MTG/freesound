...
class AdminUserForm(UserChangeForm):
    def clean_username(self):
        ...
        raise ValidationError(
            "This username is already taken or has been in used in the past by this or some other user."
        )
    ...
    def delete_spammer(self, request, obj):
        ...
        if request.method == "POST":
            ...
            messages.add_message(
                request,
                messages.INFO,
                "User '%s' will be deleted asynchronously including sounds and all of its related content." % username,
            )
            return HttpResponseRedirect(reverse("admin:auth_user_changelist"))
    ...
    def full_delete(self, request, obj):
        ...
        if request.method == "POST":
            ...
            messages.add_message(
                request,
                messages.INFO,
                "User '%s' will be deleted asynchronously including sounds and all of its relatedcontent." % username,
            )
            return HttpResponseRedirect(reverse("admin:auth_user_changelist"))
