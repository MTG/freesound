...
class UserDelete(TestCase):
    ...
    def test_delete_user_reasons(self):
        ...
        for reason in [DeletedUser.DELETION_REASON_SELF_DELETED, DeletedUser.DELETION_REASON_DELETED_BY_ADMIN,
...
class ChangeUsernameTest(TestCase):
    ...
    def test_change_username_form_profile_page(self):
        ...
        resp = self.client.post(
            reverse('accounts-edit'), data={
                'profile-username': ['userA'],
                'profile-ui_theme_preference': 'f'
            }
        )
        ...
        resp = self.client.post(
            reverse('accounts-edit'), data={
                'profile-username': [userB.username],
                'profile-ui_theme_preference': 'f'
            }
        )
        self.assertEqual(resp.status_code, 200)
        ...
        resp = self.client.post(
            ...
            data={
                'profile-username': ['userANewNewName'],
                'profile-ui_theme_preference': 'f'
            }
        )
        self.assertRedirects(resp, reverse('accounts-edit'))
        ...
        resp = self.client.post(
            reverse('accounts-edit'),
            data={
                'profile-username': ['userANewNewNewName'],
                'profile-ui_theme_preference': 'f'
            }
        )
        ...
        self.assertEqual(
            OldUsername.objects.filter(user=userA).count(), 2
        )    # ...and no new OldUsername objects created
