...
urlpatterns = [
    path(
        ...
        {"template_name": "registration/login.html", "authentication_form": FsAuthenticationForm},
        name="login",
    ),
    ...
    path(
        "password_reset/done/",
        redirect_inline(auth_views.PasswordResetDoneView.as_view(), redirect_url_name="front-page"),
    ...
    re_path(
        r"^activate/(?P<username>[^\/]+)/(?P<uid_hash>[^\/]+)/.*$",
        login_redirect(accounts.activate_user),
    ...
    re_path(
        r"^resetemail/complete/(?P<uidb36>[0-9A-Za-z]+)-(?P<token>.+)/$",
        accounts.email_reset_complete,
        name="accounts-email-reset-complete",
    ),
    ...
    path(
        "bookmarks/get_form_for_sound/<int:sound_id>/",
        bookmarks.get_form_for_sound,
        name="bookmarks-add-form-for-sound",
    ),
    path(
        "bookmarks/category/<int:category_id>/delete/",
        bookmarks.delete_bookmark_category,
        name="delete-bookmark-category",
    ),
    path("bookmarks/<int:bookmark_id>/delete/", bookmarks.delete_bookmark, name="delete-bookmark"),
    path(
        "bookmarks/category/<int:category_id>/edit_modal/",
        bookmarks.edit_bookmark_category,
        name="edit-bookmark-category",
    ),
    path(
        "bookmarks/category/<int:category_id>/download/",
        bookmarks.download_bookmark_category,
        name="download-bookmark-category",
    ),
    path(
