...
def handle_uploaded_image(profile, f):
    ...
    upload_logger.info(f"\thandling profile image upload, user {user_id}, original filename {f.name}")
    ...
    try:
        ...
        upload_logger.info("\t image upload (%s) - failed writing file error: %s", user_id, str(e))
    ...
    try:
        ...
        upload_logger.info("\t image upload (%s) - failed creating small thumbnails: %s", user_id, str(e))

    try:
        extract_square(tmp_image_path, path_m, 40)
        upload_logger.info("\t image upload (%s) - created medium thumbnail", user_id)
    except Exception as e:
        upload_logger.info("\t image upload (%s) - failed creating medium thumbnails: %s", user_id, str(e))

    try:
        extract_square(tmp_image_path, path_l, 70)
        upload_logger.info("\t image upload (%s) - created large thumbnail", user_id)
    except Exception as e:
        upload_logger.info("\t image upload (%s) - failed creating large thumbnails: %s", user_id, str(e))

    try:
        extract_square(tmp_image_path, path_xl, 100)
        upload_logger.info("\t image upload (%s) - created extra-large thumbnail", user_id)
    except Exception as e:
        upload_logger.info("\t image upload (%s) - failed creating extra-large thumbnails: %s", user_id, str(e))
...
def charts(request):
    ...
    and cache them.
    """
    tvars = cache_persistent.get(settings.CHARTS_DATA_CACHE_KEY, None)
    if tvars is None:
...
def handle_uploaded_file(user_id, f):
    ...
    upload_logger.info(f"handling file upload from user {user_id}, filename {f.name} and saving to {dest_path}")
    starttime = time.time()
