...
def merge_all(search_form, target_file=None, extra_parameters=None):
    ...
    return (
        combined_ids[
            (search_form.cleaned_data["page"] - 1) * search_form.cleaned_data["page_size"] : search_form.cleaned_data[
                "page"
            ]
    ...
    if search_form.cleaned_data["target"] or target_file:
        ...
        if len(gaia_ids) < solr_filter_id_block_size:
            ...
            solr_ids, solr_count = get_solr_results(
                search_form, page_size=len(gaia_ids), max_pages=1, valid_ids=gaia_ids, offset=last_retrieved_solr_id_pos
            )
