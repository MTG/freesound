...
examples = {
    ...
    'TextSearch': [
        (
            'Simple search', [
                'apiv2/search/text/?query=cars', 'apiv2/search/text/?query=piano&page=2',
                'apiv2/search/text/?query=bass -drum', 'apiv2/search/text/?query="bass drum" -double'
            ]
        ),
        (
            'Search with a filter', [
                'apiv2/search/text/?query=music&filter=tag:guitar',
                'apiv2/search/text/?query=music&filter=type:(wav OR aiff)',
                'apiv2/search/text/?query=music&filter=tag:bass tag:drum',
                'apiv2/search/text/?query=music&filter=tag:bass description:"heavy distortion"',
                'apiv2/search/text/?query=music&filter=is_geotagged:true tag:field-recording duration:[60 TO 120]',
                'apiv2/search/text/?query=music&filter=samplerate:44100 type:wav channels:2',
                'apiv2/search/text/?query=music&filter=duration:[0.1 TO 0.3] avg_rating:[3 TO *]'
            ]
        ),
        (
            'Simple search and selection of sound fields to return in the results', [
                'apiv2/search/text/?query=alarm&fields=name,previews',
                'apiv2/search/text/?query=alarm&fields=name,previews,analysis&descriptors=lowlevel.spectral_centroid.mean,lowlevel.pitch.mean',
                'apiv2/search/text/?query=loop&fields=uri,analysis&descriptors=rhythm.onset_times'
            ]
        ),
        ...
        (
            ...
            ['apiv2/search/text/?filter=is_geotagged:1 tag:field-recording&fields=geotag,tags']
        ),
        (
            'Basic geospatial filtering', [
                'apiv2/search/text/?filter=geotag:"Intersects(-74.093 41.042 -69.347 44.558)"',
                'apiv2/search/text/?filter=geotag:"IsDisjointTo(-74.093 41.042 -69.347 44.558)"'
            ]
        ),
        (
        ...
        (
            ...
            [
                'apiv2/search/text/?filter={!geofilt sfield=geotag pt=41.3833,2.1833 d=10} tag:barcelona&fields=id,geotag,tags',
            ]
        ),
    ],
    'ContentSearch': [
    ...
    'SoundInstance': [
        ...
        (
            'Complete sound information plus some descriptors',
            ['apiv2/sounds/213524/?descriptors=lowlevel.mfcc,rhythm.bpm']
        ),
        ('Getting only id and tags for a particular sound', ['apiv2/sounds/1234/?fields=id,tags']),
        (
            'Getting sound name and spectral centroid values (second example gets normalized centroid values)', [
                'apiv2/sounds/1234/?fields=name,analysis&descriptors=lowlevel.spectral_centroid',
                'apiv2/sounds/1234/?fields=name,analysis&descriptors=lowlevel.spectral_centroid&normalized=1'
            ]
        ),
    ],
    'SoundAnalysis': [
        ...
        (
            ...
            ['apiv2/sounds/1234/analysis/?descriptors=lowlevel.mfcc.mean&normalized=1']
        ),
    ],
    'SimilarSounds': [(
        'Getting similar sounds', [
            'apiv2/sounds/80408/similar/', 'apiv2/sounds/80408/similar/?page=2',
            'apiv2/sounds/1234/similar/?fields=name,analysis&descriptors=lowlevel.pitch.mean&descriptors_filter=.lowlevel.pitch.mean:[90 TO 110]'
        ]
    ),],
    'SoundComments': [('Get sound comments', ['apiv2/sounds/14854/comments/', 'apiv2/sounds/14854/comments/?page=2']),],
    'DownloadSound': [
    ...
    'UploadSound': [
        ...
        (
            'Upload and describe a sound all at once', [
                'curl -X POST -H "Authorization: Bearer {{access_token}}" -F audiofile=@"/path/to/your_file.wav" -F "tags=field-recording birds nature h4n" -F "description=This sound was recorded...<br>bla bla bla..." -F "license=Attribution" \'%s/apiv2/sounds/upload/\''
            ]
        ),
        (
            'Upload and describe a sound with name, pack and geotag', [
                'curl -X POST -H "Authorization: Bearer {{access_token}}" -F audiofile=@"/path/to/your_file.wav" -F "name=Another cool sound" -F "tags=field-recording birds nature h4n" -F "description=This sound was recorded...<br>bla bla bla..." -F "license=Attribution" -F "pack=A birds pack" -F "geotag=2.145677,3.22345,14" \'%s/apiv2/sounds/upload/\''
            ]
        ),
    ],
    'PendingUploads': [(
    ...
    'DescribeSound': [
        (
            'Describe a sound (only with required fields)', [
                'curl -X POST -H "Authorization: Bearer {{access_token}}" --data "upload_filename=your_file.wav&tags=field-recording birds nature h4n&description=This sound was recorded...<br>bla bla bla...&license=Attribution" \'%s/apiv2/sounds/describe/\''
            ]
        ),
        (
            'Also add a name to the sound', [
                'curl -X POST -H "Authorization: Bearer {{access_token}}" --data "upload_filename=your_file.wav&name=A cool bird sound&tags=field-recording birds nature h4n&description=This sound was recorded...<br>bla bla bla...&license=Attribution" \'%s/apiv2/sounds/describe/\''
            ]
        ),
        (
            'Include geotag and pack information', [
                'curl -X POST -H "Authorization: Bearer {{access_token}}" --data "upload_filename=your_file.wav&name=A cool bird sound&tags=field-recording birds nature h4n&description=This sound was recorded...<br>bla bla bla...&license=Attribution&pack=A birds pack&geotag=2.145677,3.22345,14" \'%s/apiv2/sounds/describe/\''
            ]
        ),
    ],
    ...
    # Users
    'UserInstance': [('User information', ['apiv2/users/reinsamba/', 'apiv2/users/Freed/']),],
    'UserSounds': [(
        'Getting user sounds', [
            'apiv2/users/Jovica/sounds/', 'apiv2/users/Jovica/sounds/?page=2',
            'apiv2/users/Jovica/sounds/?fields=id,bitdepth,type,samplerate'
        ]
    ),],
    'UserPacks': [('Getting user packs', ['apiv2/users/reinsamba/packs/', 'apiv2/users/reinsamba/packs/?page=2']),],

    # Packs
    'PackInstance': [('Getting a pack', ['apiv2/packs/9678/']),],
    'PackSounds': [('Getting pack sounds', ['apiv2/packs/9678/sounds/', 'apiv2/packs/9678/sounds/?fields=id,name']),],
    'DownloadPack': [
