...
class ApiV2ClientForm(forms.Form):
    ...
    redirect_uri = forms.URLField(
        ...
        label="Callback URL",
        max_length=200,
        widget=forms.TextInput(attrs={"style": "width:100%", "placeholder": "OAuth2 callback URL"}),
    ...
    description = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={
                "style": "width:100%",
                "placeholder": "Tell us something about what you're planning to do with this "
...
class SoundCombinedSearchFormAPI(forms.Form):
    ...
    def clean_query(self):
        ...
        if (
            unquote(query).replace('"', "").isspace()
            or unquote(query).replace('"', "") == ""
            or query.replace("'", "").isspace()
            or query == "''"
        ):
            return ""
        return query
    ...
    def construct_link(self, base_url, page=None, filt=None, group_by_pack=None, include_page=True):
        ...
        if self.cleaned_data["page_size"] and not self.cleaned_data["page_size"] == settings.APIV2["PAGE_SIZE"]:
