...
def cluster_sounds(cache_key, sound_ids, similarity_vectors_map=None):
    ...
    """
    workers_logger.info("Start clustering sounds (%s)" % json.dumps({
        'task_name': 'cluster_sounds', 
        'num_sounds': len(sound_ids)})) 
    start_time = time.time()
    
    cluster_points_results = cluster_sounds.engine.cluster_points(cache_key, sound_ids, similarity_vectors_map=similarity_vectors_map)
    
    workers_logger.info("Finished clustering sounds (%s)" % json.dumps(
        {'task_name': 'cluster_sounds', 
         'num_sounds': len(sound_ids), 
         'num_clusters': len(cluster_points_results['clusters']) if cluster_points_results['clusters'] is not None else 0,
