...
Pool FreesoundExtractor::computeAggregation(Pool& pool){
  ...
  const char* defaultStats[] = { 
    "mean", "var", "min", "max", "dmean", "dmean2", "dvar", "dvar2" };
...
void FreesoundExtractor::computeAudioMetadata(const string& audioFilename, Pool& results) {
  ...
  loudness->output("integratedLoudness") >> NOWHERE; //PC(results, "lowlevel.loudness_ebu128.integrated");
  loudness->output("momentaryLoudness") >> NOWHERE; //PC(results, "lowlevel.loudness_ebu128.momentary");
  loudness->output("shortTermLoudness") >> NOWHERE; //PC(results, "lowlevel.loudness_ebu128.short_term");
  loudness->output("loudnessRange") >> NOWHERE; //PC(results, "lowlevel.loudness_ebu128.loudness_range");
