...
def forum(request, forum_name_slug):
    ...
    threads = (
        forum.thread_set.filter(first_post__moderation_state="OK")
        .select_related(
            "last_post",
            "last_post__author",
        ...
        .prefetch_related(
            Prefetch(
                "post_set",
                queryset=Post.objects.filter(moderation_state="OK").select_related("author", "author__profile"),
            )
        )
    )
...
def thread(request, forum_name_slug, thread_id):
    ...
    paginator = paginate(
        ...
        Post.objects.select_related(
            ...
            "thread",
            "thread__forum",
        ).filter(thread=thread, moderation_state="OK"),
...
def reply(request, forum_name_slug, thread_id, post_id=None):
    ...
    latest_posts = (
        ...
        .order_by("-created")
        .filter(thread=thread, moderation_state="OK")[0:15]
    )
    ...
    if request.method == "POST":
        ...
        if user_can_post_in_forum and not user_is_blocked_for_spam_reports:
            if form.is_valid():
                may_be_spam = text_may_be_spam(form.cleaned_data.get("body", "")) or text_may_be_spam(
                ...
                if not set_to_moderation:
                    users_to_notify = []
                    for subscription in Subscription.objects.filter(thread=thread, is_active=True).exclude(
                ...
                if not set_to_moderation:
                    ...
                    messages.add_message(
                        request, messages.INFO, "Your post won't be shown until it is manually approved by moderators"
                    )
    ...
    if request.method == "POST":
        ...
        if user_can_post_in_forum and not user_is_blocked_for_spam_reports:
            if form.is_valid():
                ...
                may_be_spam = text_may_be_spam(post_body) or text_may_be_spam(post_title)
...
def subscribe_to_thread(request, forum_name_slug, thread_id):
    ...
    messages.add_message(
        request,
        messages.INFO,
        "You have been subscribed to this thread. You will receive an "
