...
const initBookmarkFormModal = (soundId, addBookmarkUrl) => {
  ...
  const categorySelectElement = document.getElementById(
    `id_${soundId.toString()}-category`
  );
  const newCategoryNameElement = document.getElementById(
    `id_${soundId.toString()}-new_category_name`
  );
  ...
  saveButtonElement.addEventListener('click', e => {
    e.preventDefault();
    const data = {};
    data.category = document.getElementById(
      `id_${soundId.toString()}-category`
    ).value;
    data.new_category_name = document.getElementById(
      `id_${soundId.toString()}-new_category_name`
    ).value;
...
const bindBookmarkSoundModals = container => {
  const bookmarkSoundButtons = [
    ...container.querySelectorAll('[data-toggle="bookmark-modal"]'),
  ];
  bookmarkSoundButtons.forEach(element => {
    ...
    element.addEventListener('click', evt => {
      ...
      const soundId = parseInt(
        modalUrlSplitted[modalUrlSplitted.length - 2],
        10
      );
      if (!evt.altKey) {
        handleGenericModal(
