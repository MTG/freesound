...
const makeCarousels = container => {
  const carouselContainers = [
    ...container.getElementsByClassName('bw-carousel-container'),
  ];
  carouselContainers.forEach(carouselContainer => {
    ...
    if (carouselContainer.dataset.carouselType === 'adaptive') {
      ...
      const elementsPerPage = Math.round(
        rowWithElements.offsetWidth / firstElement.offsetWidth
      );
      totalPages = Math.round(
        rowWithElements.childElementCount / elementsPerPage
      );
    } else {
    ...
    const transitionWrapper = carouselContainer.getElementsByClassName(
      'bw-transition-wrapper'
    )[0]; // Might be undefined
    const setPage = desiredPage => {
      if (desiredPage < 0 || desiredPage >= totalPages) return;

      if (currentPage !== desiredPage) {
        // If changing page, stop all players in the carousel (if any)
        stopAllPlayersInContainer(carouselContainer);
      }
      ...
      if (transitionWrapper !== undefined) {
        ...
        setTimeout(() => {
          transitionWrapper.classList.remove('overflow-hidden');
        }, 300);
      }
      ...
      if (totalPages > 1) {
        ...
        if (hasDots) {
          ...
          const dotsWrapper = [...carouselContainer.children].filter(
            function (element) {
    ...
    if (hasDots) {
      ...
      if (carousel.className.match(/dots-distance-\d/)) {
        const distance = carousel.className.match(/dots-distance-(\d)/)[1];
    ...
    if (carouselContainer.dataset.carouselAutoRotateSeconds !== undefined) {
      const seconds = parseInt(
        carouselContainer.dataset.carouselAutoRotateSeconds,
        10
      );
