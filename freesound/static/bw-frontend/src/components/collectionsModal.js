...
const bindCollectionModals = container => {
  const collectionButtons = [
    ...container.querySelectorAll('[data-toggle="collection-modal"]'),
  ];
...
const handleAddMaintainersModal = (
  modalId,
  modalUrl,
  selectedMaintainersDestinationElement,
  onMaintainersSelectedCallback
) => {
  handleGenericModalWithForm(
    modalUrl,
    modalContainer => {
      ...
      inputElement.addEventListener('keypress', evt => {
        if (evt.key === 'Enter') {
          ...
          handleAddMaintainersModal(
            modalId,
            `${baseUrl}?q=${inputElement.value}`,
            selectedMaintainersDestinationElement,
            onMaintainersSelectedCallback
          );
        }
      ...
      initializeObjectSelector(objectSelectorElement, element => {
        addSelectedMaintainersButton.disabled =
          element.dataset.selectedIds == '';
      });

      const addSelectedMaintainersButton =
      ...
      addSelectedMaintainersButton.addEventListener('click', evt => {
        evt.preventDefault();
        const selectableMaintainerElements = [
        ...
        selectableMaintainerElements.forEach(element => {
          ...
          if (checkbox.checked) {
            ...
            selectedMaintainersDestinationElement.appendChild(
              element.parentNode
            );
          }
        });
    ...
    undefined,
    showToast('Maintainers added successfully'),
    showToast('There were some errors handling the modal'),
    true,
    true,
    undefined,
    false
...
const prepareAddMaintainersModalAndFields = container => {
  ...
  const addMaintainersButtons = [
    ...container.querySelectorAll('[data-toggle="add-maintainers-modal"]'),
  ];
  ...
  addMaintainersButtons.forEach(addMaintainersButton => {
    ...
    const selectedMaintainersDestinationElement =
      addMaintainersButton.parentNode.parentNode.querySelector(
        '.bw-object-selector-container[data-type="users"]'
      );
    ...
    const maintainersInput =
      selectedMaintainersDestinationElement.parentNode.parentNode.getElementsByTagName(
        'input'
      )[0];
    if (maintainersInput.disabled !== false) {
      ...
      const checkboxes = selectedMaintainersDestinationElement.querySelectorAll(
    ...
    removeMaintainersButton.addEventListener('click', evt => {
      evt.preventDefault();
      const maintainerCheckboxes =
        selectedMaintainersDestinationElement.querySelectorAll(
          'input.bw-checkbox'
        );
      ...
      const selectedMaintainersHiddenInput = document.getElementById(
        addMaintainersButton.dataset.selectedMaintainersHiddenInputId
      );
      selectedMaintainersHiddenInput.value =
        selectedMaintainersDestinationElement.dataset.unselectedIds;
      removeMaintainersButton.disabled = true;
    });
    ...
    addMaintainersButton.addEventListener('click', evt => {
      ...
      handleAddMaintainersModal(
        'addMaintainersModal',
        addMaintainersButton.dataset.modalUrl,
        selectedMaintainersDestinationElement,
        selectedMaintainersIds => {
          const selectedMaintainersHiddenInput = document.getElementById(
            addMaintainersButton.dataset.selectedMaintainersHiddenInputId
          );
          const currentMaintainersIds = serializedIdListToIntList(
            selectedMaintainersHiddenInput.value
          );
          const newMaintainersIds = serializedIdListToIntList(
            selectedMaintainersIds
          );
          const combinedIds = combineIdsLists(
            currentMaintainersIds,
            newMaintainersIds
          );
          ...
          initializeObjectSelector(
            selectedMaintainersDestinationElement,
            element => {
              removeMaintainersButton.disabled =
                element.dataset.selectedIds == '';
            }
          );
        }
