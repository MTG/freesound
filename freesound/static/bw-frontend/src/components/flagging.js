...
function post_flag(div_id, flag_type, object_id, url) {
  ...
  makePostRequest(
    url,
    { flag_type: flag_type, object_id: object_id },
    () => {
      const linkElement = document.getElementById(div_id.toString() + '_link');
      const wheelElement = document.getElementById(
        div_id.toString() + '_wheel'
      );
...
const bindFlagUserButtons = container => {
  ...
  flagUserElements.forEach(element => {
    element.addEventListener('click', () => {
      post_flag(
        element.dataset.contentObjId + element.dataset.flagType,
        element.dataset.flagType,
        element.dataset.contentObjId,
        element.dataset.postFlagUrl
      );
    });
  });
};
