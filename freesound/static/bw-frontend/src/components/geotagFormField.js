...
const showGeotagField = geotagFieldElement => {
  const mapContainer =
    geotagFieldElement.getElementsByClassName('geotag-picker-map')[0];
  ...
  const removeGeotagCheckbox = document.getElementsByName(
    `${mapIdx}-remove_geotag`
  )[0];
  removeGeotagCheckbox.value = false;

  if (geotagFieldElement.map == undefined) {
    // Create map
    geotagFieldElement.map = makeGeotagEditMap(
      mapContainer.id,
      onBoundsChange,
      center_lat,
      center_lon,
    ...
    const removeGeolocationButton = geotagFieldElement.getElementsByClassName(
      'remove-geolocation-button'
    )[0];
    removeGeolocationButton.addEventListener('click', evt => {
      evt.preventDefault(); // Don't submit form
      const geotagFieldWrapper = geotagFieldElement.getElementsByClassName(
        'geotag-field-wrapper'
      )[0];
      geotagFieldWrapper.classList.add('display-none');
      const buttonElement =
        geotagFieldElement.getElementsByTagName('button')[0];
...
const showGeotagFieldAndRemoveShowButton = (
  geotagFieldElement,
  buttonElement
) => {
  const geotagFieldWrapper = geotagFieldElement.getElementsByClassName(
