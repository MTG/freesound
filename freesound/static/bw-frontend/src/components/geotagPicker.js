...
const showGeotagPickerHelpTool = refElementID => {
  ...
  if (geotagPickerMapElement !== undefined) {
    ...
    lowerButtonsElement.innerHTML =
      '<i>latitude, longitude, zoom</i>: <input id="geotag_picker_out" size="24" type="text"></input>';
    ...
    clopyToClipboardElement.onclick = () => {
      copyValue();
    };
...
const showLatLonZoomFields = (
  ...
  geotagPickerOurElement.value =
    lat.toFixed(6) + ', ' + lon.toFixed(6) + ', ' + zoom.toFixed(0);
};
