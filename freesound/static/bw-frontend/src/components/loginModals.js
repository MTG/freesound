...
const customProblemsLoggingInSubmit = event => {
  const problemsLoggingInForm = document.getElementById(
    'problemsLoggingInModalForm'
  );
  ...
  req.onload = () => {
    if (req.status >= 200 && req.status < 300) {
      showToast(
        "If the address you've entered is correct, you should now receive an email with instructions"
      );
    }
  };
  req.onerror = () => {
...
const initRegistrationForm = registrationForm => {
  ...
  registrationForm.action =
    registrationForm.action + '&next=' + encodeURI(pathWithParameters);

  // Add event handler to check username availability on focusout
  const usernameInputElement = registrationForm.querySelector(
    'input[name="username"]'
  );
  usernameInputElement.addEventListener('focusout', () => {
    checkUsernameAvailability(
      usernameInputElement.value,
      registrationForm.dataset.checkUsernameUrl,
      isAvailable => {
        const previousElementIsErrorlist =
          usernameInputElement.previousElementSibling.classList.contains(
            'errorlist'
          );
        if (isAvailable === true) {
          ...
          if (!previousElementIsErrorlist) {
            usernameInputElement.insertAdjacentHTML(
              'beforebegin',
              '<ul class="errorlist"><li>You cannot use this username to create an account</li></ul>'
            );
          }
        }
      }
...
const initLoginForm = loginForm => {
  ...
  const pathWithParameters = window.location.pathname + window.location.search;
  const loginNextHiddenInput =
    loginForm.querySelectorAll('input[name="next"]')[0];
...
const initLoginAndRegistrationModalLinks = modalContainerId => {
  ...
  [
    ...modalContainer.querySelectorAll('[data-link="problemsLoggingInModal"]'),
  ].forEach(link => {
  ...
  [...modalContainer.querySelectorAll('[data-link="loginModal"]')].forEach(
    link => {
      link.addEventListener('click', () => {
        dismissModal(modalContainerId);
        handleLoginModal();
  ...
  [...modalContainer.querySelectorAll('[data-link="registerModal"]')].forEach(
    link => {
      link.addEventListener('click', () => {
        dismissModal(modalContainerId);
...
const handleProblemsLoggingInModal = () => {
  ...
  initProblemsLoggingInForm(
    document.getElementById('problemsLoggingInModalForm')
  );
};

const handleRegistrationModal = () => {
  handleGenericModalWithForm(
    '/home/registration_modal/',
...
[...document.querySelectorAll('[data-toggle="login-modal"]')].forEach(
  modalToggle => {
    modalToggle.addEventListener('click', () => handleLoginModal());
  }
);

[...document.querySelectorAll('[data-toggle="registration-modal"]')].forEach(
  modalToggle => {
...
[
  ...document.querySelectorAll('[data-toggle="problems-logging-in-modal"]'),
].forEach(modalToggle => {
