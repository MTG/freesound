  ...
  getSoundsLocations(geotags_url, function (data) {
    ...
    if (nSounds > 0 || show_if_empty) {
      ...
      if (show_search === true) {
        map.addControl(
          new MapboxGeocoder({
            ...
            minLength: MIN_INPUT_CHARACTERS_FOR_GEOCODER,
          }),
          'top-left'
        );
      }
        ...
        map.on('click', 'sounds-unclustered', function (e) {
          ...
          if (
            ...
            ajaxLoad(url, function (data, responseCode) {
              ...
              const zoomLinkElement = document.getElementById(
                'infoWindowZoomLink-' + sound_id
              );
              zoomLinkElement.onclick = () => {
                setMaxZoomCenter(
                  zoomLinkElement.dataset.lat,
                  zoomLinkElement.dataset.lon,
                  zoomLinkElement.dataset.zoom
                );
              };

              // Init sound player inside popup
              initializeStuffInContainer(
                document.getElementById('infoWindowPlayerWrapper-' + sound_id),
                true,
                false
              );
            });
        ...
        if (center_lat === undefined) {
          ...
          if (nSounds > 1) {
            ...
            map.fitBounds(bounds, {
              duration: 0,
              offset: [0, 0],
              padding: { top: 60, right: 60, left: 60, bottom: 60 },
            });
          } else {
            map.setZoom(3);
            if (nSounds > 0) {
      ...
      map.on('style.load', function () {
        // Triggered when `setStyle` is called, add all data layers
...
function makeGeotagEditMap(
  ...
  map.on('load', function () {
    ...
    map.on('move', function (e) {
      if (
        on_bounds_changed_callback !== undefined &&
        e.triggeredFromForm !== true
      ) {
        call_on_bounds_chage_callback(
          map,
          map_element_id,
...
const makeStaticMap = (mapWrapperElementId, width, height, onclick) => {
  ...
  JSON.parse(mapWrapperElement.dataset.pins).forEach(pin => {
    pins.push(
      `url-https://freesound.org${MAP_MARKER_2X_URL}(${pin.lon},${pin.lat})`
    );
  });
 ...
 */
const makeSoundsMapWithStaticMapFirst = (
  mainWrapperElementId,
  mapCanvasId,
  staticMapWrapperElementId
) => {
  ...
  loadButtonWrapper.classList.add(
    'middle',
    'center',
    'sidebar-map',
    'border-radius-5',
