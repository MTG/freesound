...
const bindModalActivationElements = (
  querySelectorStr,
  handleModalFunction,
  container
) => {
  container.querySelectorAll(querySelectorStr).forEach(element => {
    ...
    element.addEventListener('click', evt => {
      ...
      handleModalFunction(
        ...
        element.dataset.modalActivationParam,
        element
      );
    });
  });
...
const initModalDismissButton = modalContainerElement => {
  const modalDismiss = [
    ...modalContainerElement.querySelectorAll('[data-dismiss="modal"]'),
  ];
  modalDismiss.forEach(dismiss => {
    dismiss.addEventListener('click', () =>
      dismissModal(modalContainerElement.id)
    );
  });
};
...
const initConfirmationModalDoubleConfirm = () => {
  ...
  confirmationModalAcceptForm.addEventListener('submit', evt => {
    const doubleConfirmText =
      confirmationModalAcceptForm.dataset.doubleConfirmText;
...
const bindConfirmationModalElements = container => {
  ...
  [...container.querySelectorAll('[data-toggle="confirmation-modal"]')].forEach(
    modalButton => {
      modalButton.addEventListener('click', () => {
        ...
        confirmationModalTitle.innerText =
          modalButton.dataset.modalConfirmationTitle;
        ...
        confirmationModalAcceptForm.action =
          modalButton.dataset.modalConfirmationUrl;
        confirmationModalAcceptForm.dataset.doubleConfirmText =
          modalButton.dataset.modalConfirmationDoubleConfirmText || '';
...
const handleDefaultModalWithForm = (
  modalUrl,
  modalActivationParam,
  element
) => {
  handleGenericModalWithForm(
    ...
    req => {
      showToast(
        element.dataset.successMessage || 'Form submitted successfully!'
      );
    },
    req => {
      showToast(
        element.dataset.errorMessage ||
          'There were errors processing the form...'
      );
    },
    true,
    true,
    modalActivationParam,
    element.dataset.reloadOnSuccess === 'true'
  );
};
...
const bindDefaultModals = container => {
  ...
  bindModalActivationElements(
    '[data-toggle="modal-default-with-form"]',
    handleDefaultModalWithForm,
    container
  );
};
    ...
    if (req.status >= 200 && req.status < 300) {
      if (req.responseText !== '') {
        ...
        const modalContainerId =
          genericModalWrapper.getElementsByClassName('modal')[0].id;
        const modalContainer = document.getElementById(modalContainerId);
        ...
        const modalDismiss = [
          ...modalContainer.querySelectorAll('[data-dismiss="modal"]'),
        ];
        modalDismiss.forEach(dismiss => {
          dismiss.addEventListener('click', () => {
        ...
        if (modalActivationParam !== undefined) {
          const modalAjaxRequestSearchParams = new URLSearchParams(
            fetchContentUrl
          );
      ...
      showToast(
        'Unexpected errors occurred while loading the requested content. Please try again later...'
      );
    }
  };
  req.onerror = () => {
...
const handleGenericModalWithForm = (
  fetchContentUrl,
  onLoadedCallback,
  onClosedCallback,
  onFormSubmissionSucceeded,
  onFormSubmissionError,
  doRequestAsync,
  showLoadingToast,
  modalActivationParam,
  triggerPageReloadOnSuccess
) => {
  ...
  const genericModalWithFormCustomSubmit = evt => {
    ...
    req.onload = () => {
      if (req.status >= 200 && req.status < 300) {
        if (req.getResponseHeader('content-type') === 'application/json') {
          ...
          const modalDismiss = genericModalWrapper.querySelectorAll(
            '[data-dismiss="modal"]'
          );
          modalDismiss.forEach(dismiss => {
            dismiss.addEventListener('click', () => {
    ...
    req.onerror = () => {
      ...
      if (onFormSubmissionError !== undefined) {
        ...
        showToast(
          'Unexpected errors occurred while processing the form, please try again later...'
        );
      }
    };
