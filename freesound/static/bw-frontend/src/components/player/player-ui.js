...
const updateProgressBarIndicator = (
  parentNode,
  audioElement,
  progressPercentage
) => {
  const progressBar = parentNode.getElementsByClassName(
    'bw-player__progress-bar'
  )[0];
  if (progressBar !== undefined) {
    // progress bar is only there in big players
    const progressBarIndicatorGhost = progressBar.getElementsByClassName(
      'bw-player__progress-bar-indicator--ghost'
    )[0];
    const progressBarTime = progressBar.getElementsByClassName(
      'bw-player__progress-bar-indicator--time'
    )[0];
    const progressBarIndicator = progressBar.getElementsByClassName(
  ...
  progressIndicatorContainer.addEventListener(
    ...
    throttle(evt => {
      if (parentNode.dataset.rulerActive) {
        ...
        const showingSpectrogram =
          playerImgNode.src.indexOf(parentNode.dataset.waveform) === -1;
        let readout = '';
        ...
        if (showingSpectrogram) {
          ...
          readout =
            (
              srScaleCorrection *
              rulerFrequencyMapping[Math.floor((posY / imgHeight) * 500.0)]
            ).toFixed(2) + ' Hz';
        } else {
          if (posY == height2) readout = '-inf';
          else
            readout = (
              (20 * Math.log(Math.abs(posY / height2 - 1))) /
              Math.LN10
            ).toFixed(2);
          readout = readout + ' dB';
        }
        const rulerIndicator = playerImgNode.parentNode.getElementsByClassName(
...
const createProgressStatus = (
  ...
  if (playerSize === 'big') {
    ...
    if (startWithSpectrum) {
      progressStatusContainer.classList.add(
  ...
  progressIndicator.innerHTML = formatAudioDuration(
    0,
    parentNode.dataset.showMilliseconds
  );
...
const createPlayButton = (audioElement, playerSize) => {
  ...
  playButton.addEventListener('pointerup', evt => {
    evt.stopPropagation();
  });
...
const toggleSpectrogramWaveform = (
  playerImgNode,
  waveform,
  spectrum,
  playerSize
) => {
  const controlsElement = playerImgNode.parentElement.querySelector(
    '.bw-player__controls'
  );
  const progressStatusContainerElement =
    playerImgNode.parentElement.querySelector('.bw-player__progress-container');
  const topControlsElement = playerImgNode.parentElement.querySelector(
    '.bw-player__top_controls'
  );
  const topControlsLeftElement = playerImgNode.parentElement.querySelector(
    '.bw-player__top_controls_left'
  );
  const bookmarkElement = playerImgNode.parentElement.querySelector(
    '.bw-player__favorite'
  );
  const similarSoundsElement = playerImgNode.parentElement.querySelector(
    '.bw-player__similar'
  );
  const remixSoundsElement =
    playerImgNode.parentElement.querySelector('.bw-player__remix');
  ...
  if (hasWaveform) {
    ...
    if (progressStatusContainerElement !== null) {
      progressStatusContainerElement.classList.add(
        'bw-player__progress-container--inverted'
      );
    }
  } else {
    ...
    if (progressStatusContainerElement !== null) {
      progressStatusContainerElement.classList.remove(
        'bw-player__progress-container--inverted'
      );
    }
  }
...
const createRulerButton = parentNode => {
  ...
  rulerButton.addEventListener('click', evt => {
    ...
    const rulerIndicator = parentNode.getElementsByClassName(
      'bw-player__ruler-indicator'
    )[0];
    if (parentNode.dataset.rulerActive) {
...
const createPlayerImage = (parentNode, audioElement, playerSize) => {
  ...
  if (playerSize !== 'minimal') {
    const startWithSpectrum =
      document.cookie.indexOf('preferSpectrogram=yes') > -1;
    ...
    const progressStatus = createProgressStatus(
      parentNode,
      audioElement,
      playerSize,
      startWithSpectrum
    ...
    imageContainer.addEventListener('pointerup', evt => {
      // We use "pointerup" here so we can distinguish between mouse and touch events
      if (evt.altKey) {
        ...
        if (playerSize === 'small') {
          if (
            ...
            !parentNode.classList.contains('last-played') &&
            audioElement.paused
          ) {
...
const createPlayerControls = (
  ...
  if (playerImgNode !== undefined) {
    // Some players don't have playerImgNode (minimal)
    startWithSpectrum =
      playerImgNode.src.indexOf(parentNode.dataset.waveform) === -1;
  }
  if (startWithSpectrum) {
...
const createPlayerTopControls = (
  parentNode,
  playerImgNode,
  playerSize,
  showSimilarSoundsButton,
  showBookmarkButton,
  showRemixGroupButton,
  ...
  if (showSimilarSoundsButton) {
    const similarSoundsButton = createSimilarSoundsButton(
      parentNode,
      playerImgNode
    );
...
const createCollectionButton = (parentNode, playerImgNode) => {
  ...
  collectionButtonContainer.addEventListener('pointerup', evt =>
    evt.stopPropagation()
  );
  collectionButtonContainer.addEventListener('click', evt =>
    evt.stopPropagation()
  );
  ...
  collectionButton.setAttribute(
    'data-modal-content-url',
    parentNode.dataset.collectionModalContentUrl
  );
...
const createSetFavoriteButton = (parentNode, playerImgNode) => {
  ...
  favoriteButtonContainer.setAttribute(
    'data-modal-url',
    parentNode.dataset.bookmarkModalUrl
  );
  favoriteButtonContainer.setAttribute(
    'data-add-bookmark-url',
    parentNode.dataset.addBookmarkUrl
  );
  favoriteButtonContainer.appendChild(
    getIsFavorite() ? unfavoriteButton : favoriteButton
  );

  favoriteButtonContainer.addEventListener('pointerup', evt =>
    evt.stopPropagation()
  );
...
const createSimilarSoundsButton = (parentNode, playerImgNode) => {
  ...
  similarSoundsButtonContainer.addEventListener('pointerup', evt =>
    evt.stopPropagation()
  );
  similarSoundsButtonContainer.addEventListener('click', evt =>
    evt.stopPropagation()
  );
  ...
  similarSoundsButton.setAttribute(
    'data-modal-content-url',
    parentNode.dataset.similarSoundsModalUrl
  );
...
const createRemixGroupButton = (parentNode, playerImgNode) => {
  ...
  remixGroupButtonContainer.addEventListener('pointerup', evt =>
    evt.stopPropagation()
  );
  remixGroupButtonContainer.addEventListener('click', evt =>
    evt.stopPropagation()
  );
  ...
  remixGroupButton.setAttribute(
    'data-modal-content-url',
    parentNode.dataset.remixGroupModalUrl
  );
...
const createPlayer = parentNode => {
  ...
  const playerImage = createPlayerImage(parentNode, audioElement, playerSize);
  ...
  const topControls = createPlayerTopControls(
    parentNode,
    playerImgNode,
    playerSize,
    showSimilarSoundsButton,
  ...
  const rateSoundHiddenWidget = parentNode.parentNode.getElementsByClassName(
