...
const createSedButton = (audioElement, parentNode, soundId, sedExperiment) => {
...
const createPlayerControls = (parentNode, playerImgNode, audioElement, playerSize) => {
  ...
  try {
    ...
    const soundIndex = pathSegments.indexOf('sounds');
    soundId = pathSegments[soundIndex+1];
    const sedIndex = pathSegments.indexOf('sed');
    if (sedIndex !== -1 && sedIndex < pathSegments.length -1){
      sedExperiment = parseInt(pathSegments[sedIndex +1 ]);
    } else {
      sedExperiment = 0;
    }
  } catch (e) {
  ...
  const controls =
    playerSize === 'big'
      ? [createLoopButton(audioElement),
         ...
         ...(showSedButton ? [createSedButton(audioElement, parentNode, soundId, sedExperiment)] : [])]
      : [createPlayButton(audioElement, playerSize),
         createLoopButton(audioElement)]
