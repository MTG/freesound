...
const halfStarHtml =
  '<span class="bw-icon-half-star text-red"><span class="path1"></span><span class="path2"></span></span>';
const starHtml = '<span class="bw-icon-star text-red"></span></span>';

const updateStarIconsClasses = (
  ratingContainerElement,
  newRatingValue,
  fillClass
) => {
  const newRating = parseFloat(newRatingValue);
  const relatedStarIconElements = ratingContainerElement.querySelectorAll(
    '.bw-icon-star,.bw-icon-half-star'
  );
  for (let i = 0; i < relatedStarIconElements.length; i += 1) {
    ...
    const currentStarHalfRating =
      (currentStarLowRating + currentStarHighRating) / 2;
    if (currentStarHighRating <= newRating) {
      starIconElement.outerHTML = starHtml.replace('text-red', fillClass);
    } else if (
      currentStarHalfRating <= newRating &&
      newRating < currentStarHighRating
    ) {
      ...
      starIconElement.outerHTML = starHtml.replace(
        'text-red',
        'text-light-grey'
      );
    }
...
const updateRatingWidgetInSoundPageDescriptionSection = data => {
  ...
  if (isSoundsPage) {
    const soundPageInformationElement = document.getElementsByClassName(
      'bw-sound-page__information'
    )[0];
    if (soundPageInformationElement !== undefined) {
      const ratingsContainer =
        soundPageInformationElement.getElementsByClassName(
          'bw-rating__container'
        )[0];
      if (ratingsContainer !== undefined) {
        updateStarIconsClasses(
          ratingsContainer,
          data.avg_rating / 2,
          'text-red'
        );
      }
      const ratingsCountElement =
        soundPageInformationElement.getElementsByClassName(
          'bw-rating__count'
        )[0];
...
const updateRelatedCountAndAvgIndicators = (data, ratingInput) => {
  ...
  const ratingsCountElement =
    ratingsContainer.parentNode.getElementsByClassName('bw-rating__count')[0];
  if (ratingsCountElement !== undefined) {
    ratingsCountElement.innerText = data.num_ratings_display_short;
  }
  const bwPlayer = ratingsContainer.closest('.bw-player');
  if (bwPlayer !== null) {
    const ratingsAvgElement =
      bwPlayer.parentNode.getElementsByClassName('bw-rating__avg')[0];
  ...
  req.onreadystatechange = () => {
    if (req.readyState === 4) {
      if (req.status === 200) {
        ...
        showToast(
          'There were problems entering your rating. Please try again later'
        );
      }
    }
  };
