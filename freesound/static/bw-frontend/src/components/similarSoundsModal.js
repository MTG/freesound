...
const handleSimilarSoundsModal = (modalUrl, modalActivationParam) => {
  handleGenericModal(
    ...
    modalContainer => {
      ...
      const similaritySpaceSelect = modalContainer.querySelector(
        '#similarity-space-select'
      );
      if (similaritySpaceSelect) {
        similaritySpaceSelect.addEventListener('change', evt => {
          ...
          currentUrl.searchParams.set(
            'page',
            similaritySpaceSelect.dataset.currentPage
          );
...
const bindSimilarSoundsModal = container => {
  bindModalActivationElements(
    '[data-toggle="similar-sounds-modal"]',
    handleSimilarSoundsModal,
    container
  );
};
