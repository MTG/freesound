...
const fetchTagSuggestions = async inputElement => {
  ...
  const data = await getJSONFromPostRequestWithFetch(
    inputElement.dataset.typeaheadSuggestionsUrl,
    { input_tags: allTags }
  );
  const suggestions = [];
  data[0].forEach(tag => {
    suggestions.push({
      label: '<div class="padding-1">' + tag + '</div>',
      value: tag,
    });
  });
  return suggestions;
...
const onSuggestionSelectedFromDropdown = (
  ...
  if (inputElement.focusoutTimeout !== undefined) {
    clearTimeout(inputElement.focusoutTimeout);
...
const drawWrapperContents = (
  ...
  currentTags.split(' ').forEach((tagToRender, index) => {
    if (tagToRender !== '') {
      ...
      tagElement.classList =
        'bg-white text-black border-grey-light text-center padding-1 no-text-wrap no-letter-spacing h-spacing-1 v-spacing-1';
      ...
      closeIconNode.addEventListener('click', evt => {
        ...
        const currentTagsArray =
          inputWrapperElement.dataset.currentTags.split(' ');
  ...
  if (someTagsRendered) {
    ...
    clearTagsButton.className =
      'bw-link--grey no-hover cursor-pointer h-spacing-left-1';
...
const updateTags = (inputElement, newTagsStr) => {
  ...
  const tagsHiddenInput =
    inputElement.parentNode.parentNode.parentNode.querySelectorAll(
      'input[name$="tags"]'
    )[0];
  inputWrapperElement.dataset.currentTags += ' ' + newTagsStr;
  inputWrapperElement.dataset.currentTags =
    inputWrapperElement.dataset.currentTags.replace(
      notAlphanumericDashOrSpaceRegex,
      ''
    ); // Remove non-alphanumeric, dash or space
  inputWrapperElement.dataset.currentTags =
    inputWrapperElement.dataset.currentTags.replace(multiDashesRegex, '-'); // Remove multi-dashes
  inputWrapperElement.dataset.currentTags =
    inputWrapperElement.dataset.currentTags
      .split(' ')
      .filter(item => !(item === '-'))
      .join(' '); // Remove 1 character tags which are a dash
  inputWrapperElement.dataset.currentTags =
    inputWrapperElement.dataset.currentTags.replace(/\s+/g, ' ').trim(); // Remove extra white spaces
...
const prepareTagsFormFields = container => {
  ...
  tagsInputFields.forEach(tagsFieldElement => {
    ...
    const inputWrapperElement =
      tagsFieldElement.getElementsByClassName('tags-input-wrapper')[0];
    ...
    inputElement.addEventListener('keydown', evt => {
      if (evt.key == 'Backspace' && inputElement.value.length == 0) {
        ...
        const currentTagsArray =
          inputWrapperElement.dataset.currentTags.split(' ');
        const lastIntroducedTag =
          inputWrapperElement.dataset.currentTags.split(' ')[
            currentTagsArray.length - 1
          ];
        inputWrapperElement.dataset.currentTags = currentTagsArray
          .slice(0, currentTagsArray.length - 1)
          .join(' ');
        ...
        inputElement.value = lastIntroducedTag.substring(
          0,
          lastIntroducedTag.length - 1
        );
      }
    });

    inputElement.addEventListener('paste', evt => {
      let pasteText = (evt.clipboardData || window.clipboardData).getData(
        'text'
      );
