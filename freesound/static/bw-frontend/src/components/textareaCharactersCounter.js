const updateTextareaCounter = (textareaInputElementId, value) => {
  document.getElementById(
    textareaInputElementId + '-counter-number'
...
const makeTextareaCharacterCounter = container => {
  ...
  textAreaElements.forEach(textareaInputElement => {
    ...
    if (textareaInputElement.hasAttribute('maxlength')) {
      textareaInputElement.parentNode.classList.add(
      ...
      rightSpan.innerHTML =
        '/' + textareaInputElement.getAttribute('maxlength');
      ...
      textareaInputElement.addEventListener('keyup', e =>
        updateTextareaCounter(textareaInputElement.id, e.target.value)
      );

      // Set position depending on the height of the helptext immediately below the textarea
      const helptextElement =
        textareaInputElement.parentNode.getElementsByClassName('helptext')[0];
