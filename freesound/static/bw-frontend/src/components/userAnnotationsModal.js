...
const saveAnnotation = (addAnnotationUrl, text, user_id) => {
  ...
  makePostRequest(
    ...
    responseText => {
      ...
      document
        .querySelectorAll('.annotation-label-' + user_id)
        .forEach(element => {
  ...
  handleGenericModal(
    ...
    modalContainer => {
      ...
      const saveButtonElement =
        buttonsInModalForm[buttonsInModalForm.length - 1];
      saveButtonElement.addEventListener('click', e => {
        e.preventDefault();
        saveAnnotation(
          saveButtonElement.dataset.addAnnotationUrl,
          textInputField.value,
          saveButtonElement.dataset.userId
        );
      });
    },
    undefined,
...
const bindUserAnnotationsModal = container => {
  bindModalActivationElements(
    '[data-toggle="user-annotations-modal"]',
    handleUserAnnotationsModal,
    container
  );
};
