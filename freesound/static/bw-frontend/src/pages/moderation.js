...
const includeDeferredTicketsCheckbox =
  document.getElementById('include-deferred');
...
const ticketCommentsSection = document.getElementById(
  'ticket-comments-section'
);

const closeCollapsableBlocks = soundElement => {
  const collapsableToggleElement =
    soundElement.getElementsByClassName('collapsable-toggle')[0];
  const collapsableContainer = document.getElementById(
    collapsableToggleElement.dataset.target
  );
...
const postTicketsSelected = () => {
  ...
  if (selectedTicketsData.length === 1) {
    const commentElement = ticketCommentsSection.querySelector(
      `.ticket-comments[data-ticket-id="${selectedTicketsData[0]['ticketId']}"]`
    );
  ...
  const ticketIdsSerialized = selectedTicketsData
    .map(ticketData => ticketData['ticketId'])
    .join('|');
  ...
  selectedTicketsData.forEach(ticketData => {
    const soundInfoElement = document.querySelector(
      `.sound-info-element[data-sound-id="${ticketData['soundId']}"]`
    );
    if (selectedTicketsData.length === 1) {
...
const scrollWindowToSoundInfoElement = soundId => {
  if (
    selectedSoundsInfoPanel.offsetHeight -
      parseFloat(selectedSoundsInfoPanel.style.paddingBottom) +
      moderateForm.offsetHeight >
    window.innerHeight
  ) {
    const soundInfoElement = selectedSoundsInfoPanel.querySelector(
      `.sound-info-element[data-sound-id="${soundId}"]`
    );
    if (soundInfoElement !== undefined) {
      const amountToScroll =
        soundInfoElement.offsetTop + moderateForm.offsetHeight - 180;
...
ticketCheckboxes.forEach(checkbox => {
  ...
  checkbox.addEventListener('change', () => {
    if (
      checkbox.dataset.altKey !== 'true' &&
      checkbox.dataset.shiftKey !== 'true'
    ) {
    ...
    const soundInfoElement = document.querySelector(
      `.sound-info-element[data-sound-id="${checkbox.closest('tr').dataset.soundId}"]`
    );
    ...
    if (shouldAutoscroll()) {
      if (!didMultipleSelection) {
        if (checkbox.checked) {
          scrollWindowToSoundInfoElement(
            checkbox.closest('tr').dataset.soundId
          );
        } else {
          // If unchecked, scroll a bit to top (the ammount of a sound info element)
          const currentScrollPosition = document.documentElement.scrollTop;
          document.documentElement.scrollTop = document.body.scrollTop =
            currentScrollPosition - soundInfoElement.offsetHeight;
        }
      }
    }
  });
