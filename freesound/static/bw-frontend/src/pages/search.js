...
const searchInputBrowsePlaceholder =
  searchInputBrowse.getAttribute('placeholder');
const removeSearchInputValueBrowse = document.getElementById(
  'remove-content-search'
);
const advancedSearchOptionsDiv = document.getElementById(
...
window.addEventListener('load', function () {
  updateRemoveSearchInputButtonVisibility(searchInputBrowse);
});
...
const searchFormIsVisible = () => {
  ...
  if (advancedSearchOptionsIsVisible()) {
    ...
    if (!tagsMode) {
      ...
      heroRect = document
        .getElementById('tags-mode-input-section')
...
const toggleAdvancedSearchOptionsElement = document.getElementById(
...
function updateToggleAdvancedSearchOptionsText() {
  if (advancedSearchOptionsIsVisible()) {
    toggleAdvancedSearchOptionsElement.innerHTML =
...
toggleAdvancedSearchOptionsElement.addEventListener(
  'click',
...
const advancedSearchOptionsHaveChangedSinceLastQuery = () => {
  const currentAdvancedSearchInputValues =
...
const onAdvancedSearchOptionsInputsChange = () => {
  document.getElementById('advanced-search-apply-button').disabled =
...
[...searchFormElement.getElementsByClassName('bw-checkbox')].forEach(checkbox => {
  ...
  checkbox.addEventListener('change', evt => {
    // Update hidden checkbox value when checkbox is changed
    hiddenCheckbox.value = checkbox.checked ? '1' : '0';
  });
...
document.addEventListener('async_section_loaded', () =>
  bindClusteringRadioButtonsSubmit()
);
...
if (searchPageNavbarForm !== null) {
  searchPageNavbarForm.addEventListener('submit', function (evt) {
    ...
    const searchInputBrowseNavbar = document.getElementById(
      'search-input-browse-navbar'
    );
