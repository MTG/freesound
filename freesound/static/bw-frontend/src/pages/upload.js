...
var setSizeBarValue = function setSizeBarValue(value) {
  ...
  file_size.innerText =
    Math.round((maxUploadSize / 1024 / 1024) * (1 - value / 100)) +
    ' MB remaining';
...
var addSizeBarElement = function addSizeBarElement() {
  ...
  sizeTable.innerHTML =
    '<td width=0%; id="file-size"; style="background: #cacad4" /><td style="background: #cacad4" />';
...
var onFileInputChange = function onFileInputChange(event) {
  ...
  showListOfSounds();
  var percent_size =
    (soundsQueue.reduce((acc, curr) => acc + curr.size, 0) * 100) /
    maxUploadSize;
...
var errorList =
  document.getElementsByClassName('errorlist')[0] || addErrorListElement();
var sizeBar =
  document.getElementsByClassName('sizebar')[0] || addSizeBarElement();
addFilesButton.onclick = function onclick() {
  fileInput.click();
};
