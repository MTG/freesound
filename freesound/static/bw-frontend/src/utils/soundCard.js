...
export function extractSoundFromCard(card) {
  ...
  return {
    ...
    description: (
      card.querySelector('.bw-player__description-height') || {
        textContent: '',
      }
    ).textContent.trim(),
 ...
 */
export function populateSoundCard(
  templateEl,
  sound,
  { previewsUrl, displaysUrl }
) {
  ...
  if (overflow)
    overflow.setAttribute(
      'aria-label',
      `Sound ${sound.name} by ${sound.username}`
    );
  ...
  const iconsTarget = clone.querySelector(
    `.js-small-icons-line${smallIconsLine(sound)}`
  );
...
function buildSmallIconsHtml(sound, username) {
  const soundId = Number.parseInt(sound.id, 10);
  const safeSoundId = Number.isFinite(soundId) ? soundId : 0;
  const safePackName = escapeAttr(sound.pack_name || '');
  const safeLicenseName = escapeAttr(sound.license_name || '');
  const safeLicenseIcon = String(sound.license_icon || 'cc').match(/^[a-z0-9-]+$/i)
    ? sound.license_icon
    : 'cc';
  let html = '';
  if (sound.num_downloads)
    html +=
      `<div class="h-spacing-left-1" title="${sound.num_downloads} downloads">` +
      `<a href="javascript:void(0)" data-toggle="modal-default" data-modal-content-url="/people/${username}/sounds/${safeSoundId}/downloaders/?ajax=1" class="bw-link--grey-light">` +
      `<span class="bw-icon-download" style="font-size:90%"></span> ${formatNumber(sound.num_downloads)}</a></div>`;
  if (sound.num_comments)
    html +=
      `<div class="h-spacing-left-1" title="${sound.num_comments} comments">` +
      `<a href="javascript:void(0)" data-toggle="modal-default" data-modal-content-url="/people/${username}/sounds/${safeSoundId}/comments/?ajax=1" class="bw-link--grey-light">` +
      `<span class="bw-icon-comments" style="font-size:90%"></span> ${formatNumber(sound.num_comments)}</a></div>`;
  if (sound.pack_id)
    html += `<div class="h-spacing-left-1" title="Pack: ${safePackName}"><a href="/people/${username}/packs/${sound.pack_id}/" class="bw-link--grey-light"><span class="bw-icon-stack"></span></a></div>`;
  if (sound.has_geotag)
    html +=
      `<div class="h-spacing-left-1">` +
      `<a href="javascript:void(0)" data-toggle="modal-default" data-modal-content-url="/people/${username}/sounds/${safeSoundId}/geotag/?ajax=1" class="bw-link--grey-light">` +
      `<span class="bw-icon-pin"></span></a></div>`;
  html += `<div class="h-spacing-left-1" title="License: ${safeLicenseName}"><span class="bw-icon-${safeLicenseIcon || 'cc'}"></span></div>`;
  if (sound.avg_rating != null)
    html +=
      `<div class="h-spacing-left-1" title="${sound.num_ratings} rating${sound.num_ratings !== 1 ? 's' : ''}">` +
...
function buildRatingWidgetHtml(sound) {
  ...
  const username = encodeURIComponent(sound.username);
  let html =
    '<div class="bw-rating__container" data-show-added-rating-on-save="false"' +
  ...
  for (let i = 5; i >= 1; i--) {
    ...
    html +=
      `<input class="bw-rating__input" type="radio" name="rate-${safeSoundId}"` +
      ` data-rate-url="/people/${username}/sounds/${safeSoundId}/rate/${i}/"` +
      ` id="rate-${safeSoundId}-${i}" value="${i}">`;
    html += `<label for="rate-${safeSoundId}-${i}" data-value="${i}" aria-label="Rate sound ${i} star${i !== 1 ? 's' : ''}">`;
    html += isHalf
