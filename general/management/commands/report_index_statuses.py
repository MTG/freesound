...
class Command(LoggingBaseCommand):
    help = (
        "This command checks the status of the solr and gaia index compared to the fs database. Reports about "
    ...
    def handle(self, *args, **options):
        ...
        sound_ids_analyzed_with_analyzer_ok = list(
            SoundAnalysis.objects.filter(
                analyzer=settings.FREESOUND_ESSENTIA_EXTRACTOR_NAME, analysis_status="OK"
        ...
        messages.append(
            "Sounds in fs but not in gaia:\t%i  (only considering sounds correctly analyzed)" % len(in_fs_not_in_gaia)
        )
        console_logger.info("\n".join(messages))
