...
def whitelist_user(ticket_ids=None, user_id=None):
    ...
    workers_logger.info(
        "Start whitelisting users from tickets (%s)" % json.dumps({
    ...
    workers_logger.info(
        "Finished whitelisting users from tickets (%s)" % json.dumps({
            'task_name': WHITELIST_USER_TASK_NAME,
            'n_tickets': len(ticket_ids) if ticket_ids is not None else 0,
            'user_id': user_id if user_id is not None else '',
...
def delete_user(user_id, deletion_action, deletion_reason):
    try:
        ...
        workers_logger.info(
            "Can't delete user as it does not exist (%s)" % json.dumps({
                'task_name': deletion_action,
                'user_id': user_id,
                'username': '-',
    ...
    workers_logger.info(
        "Start deleting user (%s)" % json.dumps({
            'task_name': deletion_action,
            'user_id': user_id,
    ...
    try:
        if deletion_action in [FULL_DELETE_USER_ACTION_NAME, DELETE_USER_KEEP_SOUNDS_ACTION_NAME,
            ...
            if deletion_action == DELETE_USER_KEEP_SOUNDS_ACTION_NAME:
                ...
                # being authored by a "deleted user".
                user.profile.delete_user(remove_sounds=True, deletion_reason=deletion_reason)
                ...
                # deleted.
                user.profile.delete_user(delete_user_object_from_db=True, deletion_reason=deletion_reason)
            ...
            workers_logger.info(
                "Finished deleting user (%s)" % json.dumps({
                    'task_name': deletion_action,
                    'user_id': user.id,
                    'username': username_before_deletion,
                    'deletion_reason': deletion_reason,
        ...
        workers_logger.info(
            "Unexpected error while deleting user (%s)" % json.dumps({
                'task_name': deletion_action,
                'user_id': user.id,
                'username': username_before_deletion,
                'deletion_reason': deletion_reason,
...
def validate_bulk_describe_csv(bulk_upload_progress_object_id):
    ...
    workers_logger.info(
        "Starting validation of BulkUploadProgress (%s)" % json.dumps({
    ...
    try:
        ...
        workers_logger.info(
            "Finished validation of BulkUploadProgress (%s)" % json.dumps({
                'task_name': VALIDATE_BULK_DESCRIBE_CSV_TASK_NAME,
                'bulk_upload_progress_id': bulk_upload_progress_object_id,
                'work_time': round(time.time() - start_time)
            })
        ...
        workers_logger.info(
            "Error validating of BulkUploadProgress (%s)" % json.dumps({
                'task_name': VALIDATE_BULK_DESCRIBE_CSV_TASK_NAME,
                'bulk_upload_progress_id': bulk_upload_progress_object_id,
                'error': str(e),
                'work_time': round(time.time() - start_time)
...
def bulk_describe(bulk_upload_progress_object_id):
    ...
    workers_logger.info(
        "Starting describing sounds of BulkUploadProgress (%s)" % json.dumps({
    ...
    try:
        ...
        bulk.save()
        workers_logger.info(
            "Finished describing sounds of BulkUploadProgress (%s)" % json.dumps({
                'task_name': BULK_DESCRIBE_TASK_NAME,
                'bulk_upload_progress_id': bulk_upload_progress_object_id,
                'work_time': round(time.time() - start_time)
            })
        ...
        workers_logger.info(
            "Error describing sounds of BulkUploadProgress (%s)" % json.dumps({
                'task_name': BULK_DESCRIBE_TASK_NAME,
                'bulk_upload_progress_id': bulk_upload_progress_object_id,
                'error': str(e),
                'work_time': round(time.time() - start_time)
...
def process_analysis_results(sound_id, analyzer, status, analysis_time, exception=None):
    ...
    workers_logger.info(
        "Starting processing analysis results (%s)" % json.dumps({
    ...
    try:
        ...
        if exception:
            workers_logger.info(
                "Finished processing analysis results (%s)" % json.dumps({
                    'task_name': PROCESS_ANALYSIS_RESULTS_TASK_NAME,
                    'sound_id': sound_id,
                    'analyzer': analyzer,
                    'status': status,
        ...
        workers_logger.info(
            "Error processing analysis results (%s)" % json.dumps({
                'task_name': PROCESS_ANALYSIS_RESULTS_TASK_NAME,
                'sound_id': sound_id,
                'analyzer': analyzer,
                'status': status,
...
def process_sound(sound_id, skip_previews=False, skip_displays=False):
    ...
    workers_logger.info(
        "Starting processing of sound (%s)" % json.dumps({
    ...
    try:
        ...
        if result:
            workers_logger.info(
                "Finished processing of sound (%s)" % json.dumps({
                    'task_name': SOUND_PROCESSING_TASK_NAME,
                    'sound_id': sound_id,
                    'result': 'success',
                    'work_time': round(time.time() - start_time)
        ...
        try:
            ...
            pass
        workers_logger.info(
            "WorkerException while processing sound (%s)" % json.dumps({
                'task_name': SOUND_PROCESSING_TASK_NAME,
                'sound_id': sound_id,
                'error': str(e),
                'work_time': round(time.time() - start_time)
        ...
        try:
            ...
            pass
        workers_logger.info(
            "Unexpected error while processing sound (%s)" % json.dumps({
                'task_name': SOUND_PROCESSING_TASK_NAME,
                'sound_id': sound_id,
                'error': str(e),
                'work_time': round(time.time() - start_time)
...
def process_before_description(audio_file_path):
    ...
    workers_logger.info(
        "Starting processing-before-describe of sound (%s)" % json.dumps({
    ...
    try:
        ...
        if result:
            workers_logger.info(
                "Finished processing-before-describe of sound (%s)" % json.dumps({
                    'task_name': PROCESS_BEFORE_DESCRIPTION_TASK_NAME,
                    'audio_file_path': audio_file_path,
                    'result': 'success',
                    'work_time': round(time.time() - start_time)
        ...
        workers_logger.info(
            "WorkerException while processing-before-describe sound (%s)" % json.dumps({
                'task_name': PROCESS_BEFORE_DESCRIPTION_TASK_NAME,
                'audio_file_path': audio_file_path,
                'error': str(e),
                'work_time': round(time.time() - start_time)
        ...
        workers_logger.info(
            "Unexpected error while processing-before-describe sound (%s)" % json.dumps({
                'task_name': PROCESS_BEFORE_DESCRIPTION_TASK_NAME,
                'audio_file_path': audio_file_path,
                'error': str(e),
                'work_time': round(time.time() - start_time)
