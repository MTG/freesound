...
def whitelist_user(annotation_sender_id, ticket_ids=None, user_id=None):
    ...
    workers_logger.info(
        "Start whitelisting users from tickets (%s)"
        % json.dumps(
            {
    ...
    workers_logger.info(
        "Finished whitelisting users from tickets (%s)"
        % json.dumps(
            {
                "task_name": WHITELIST_USER_TASK_NAME,
                "n_tickets": len(ticket_ids) if ticket_ids is not None else 0,
...
def post_moderation_assigned_tickets(ticket_ids=[], notification=None, users_to_update=None, packs_to_update=None):
    ...
    workers_logger.info(
        "Finished post moderation assigned tickets (%s)"
        % json.dumps(
            {
...
def delete_user(user_id, deletion_action, deletion_reason):
    try:
        ...
        workers_logger.info(
            "Can't delete user as it does not exist (%s)"
            % json.dumps(
                {"task_name": deletion_action, "user_id": user_id, "username": "-", "deletion_reason": deletion_reason}
            )
        )
        return
    ...
    try:
        ...
        workers_logger.info(
            "Unexpected error while deleting user (%s)"
            % json.dumps(
                {
                    "task_name": deletion_action,
                    "user_id": user.id,
...
def validate_bulk_describe_csv(bulk_upload_progress_object_id):
    ...
    workers_logger.info(
        "Starting validation of BulkUploadProgress (%s)"
        % json.dumps(
            {
    ...
    try:
        ...
        workers_logger.info(
            "Finished validation of BulkUploadProgress (%s)"
            % json.dumps(
                {
                    "task_name": VALIDATE_BULK_DESCRIBE_CSV_TASK_NAME,
                    "bulk_upload_progress_id": bulk_upload_progress_object_id,
        ...
        workers_logger.info(
            "Error validating of BulkUploadProgress (%s)"
            % json.dumps(
                {
                    "task_name": VALIDATE_BULK_DESCRIBE_CSV_TASK_NAME,
                    "bulk_upload_progress_id": bulk_upload_progress_object_id,
...
def bulk_describe(bulk_upload_progress_object_id):
    ...
    try:
        ...
        bulk.save()
        workers_logger.info(
            "Finished describing sounds of BulkUploadProgress (%s)"
            % json.dumps(
                {
        ...
        workers_logger.info(
            "Error describing sounds of BulkUploadProgress (%s)"
            % json.dumps(
                {
                    "task_name": BULK_DESCRIBE_TASK_NAME,
                    "bulk_upload_progress_id": bulk_upload_progress_object_id,
...
def process_analysis_results(sound_id, analyzer, status, analysis_time, exception=None):
    ...
    workers_logger.info(
        "Starting processing analysis results (%s)"
        % json.dumps(
            {
    ...
    try:
        ...
        if exception:
            workers_logger.info(
                "Finished processing analysis results (%s)"
                % json.dumps(
                    {
                        "task_name": PROCESS_ANALYSIS_RESULTS_TASK_NAME,
                        "sound_id": sound_id,
...
def process_sound(sound_id, skip_previews=False, skip_displays=False):
    ...
    try:
        ...
        result = FreesoundAudioProcessor(sound_id=sound_id).process(
            skip_displays=skip_displays, skip_previews=skip_previews
        )
        if result:
            workers_logger.info(
                "Finished processing of sound (%s)"
                % json.dumps(
                    {
        ...
        try:
            ...
            pass
        workers_logger.info(
            "WorkerException while processing sound (%s)"
            % json.dumps(
                {
                    "task_name": SOUND_PROCESSING_TASK_NAME,
                    "sound_id": sound_id,
        ...
        try:
            ...
            pass
        workers_logger.info(
            "Unexpected error while processing sound (%s)"
            % json.dumps(
                {
                    "task_name": SOUND_PROCESSING_TASK_NAME,
                    "sound_id": sound_id,
