...
def log_map_load(map_type, num_geotags, request):
    web_logger.info(
        "Map load (%s)" % json.dumps({"map_type": map_type, "num_geotags": num_geotags, "ip": get_client_ip(request)})
    )
...
def _get_geotags_query_params(request):
    return {
        ...
        "query_params": urllib.parse.unquote(request.GET["qp"])
        if "qp" in request.GET
        else None,  # This is used for map embeds based on general queries
    }
...
def geotags(request, tag=None):
    ...
    tvars.update(
        {  # Overwrite tag and username query params (if present)
            "tag": tag,
            "username": None,
            "pack": None,
            "url": url,
...
def for_pack(request, username, pack_id):
    ...
    tvars.update(
        {  # Overwrite tag and username query params (if present)
            ...
            "query_search_page_url": reverse("sounds-search")
            + f'?f=pack_grouping:"{pack.id}_{urllib.parse.quote(pack.name)}"&mm=1',
            "modal_version": request.GET.get("ajax"),
        }
    )
    if request.GET.get("ajax"):
