...
class Command(LoggingBaseCommand):
    ...
    def handle(self, **options):
        ...
        new_sounds_mod = (
            sounds.models.Sound.objects.filter(created__gt=time_span, moderation_date__isnull=False)
            .extra(select={"day": "date(moderation_date)"})
            .values("day")
            .order_by()
            .annotate(Count("id"))
        )
        ...
        new_sounds = (
            sounds.models.Sound.objects.filter(created__gt=time_span, processing_date__isnull=False)
        ...
        new_downloads_sound = (
            sounds.models.Download.objects.filter(created__gt=time_span)
        ...
        new_downloads_pack = (
            sounds.models.PackDownload.objects.filter(created__gt=time_span)
            .extra({"day": 'date("sounds_packdownload".created)'})
            .values("day")
            .order_by()
        ...
        new_users = (
            User.objects.filter(date_joined__gt=time_span)
            .extra(select={"day": "date(date_joined)"})
            .values("day", "is_active")
            .order_by()
            .annotate(Count("id"))
        )
        ...
        query_donations = (
            donations.models.Donation.objects.filter(created__gt=time_span)
            .extra({"day": "date(created)"})
            .values("day")
            .order_by()
            .annotate(Sum("amount"))
        ...
        tags_stats = (
            SoundTag.objects.values("tag_id")
            .filter(created__gt=time_span)
            .annotate(num=Count("tag_id"))
            .values("num", "tag__name")
            .order_by("-num")[:300]
        )
        ...
        num_donations = donations.models.Donation.objects.aggregate(Sum("amount"))["amount__sum"]

        time_span = timezone.now() - datetime.timedelta(30)
        sum_donations_month = donations.models.Donation.objects.filter(created__gt=time_span).aggregate(Sum("amount"))[
            "amount__sum"
        ]
