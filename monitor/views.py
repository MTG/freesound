...
def monitor_processing(request):
    ...
    sounds_pending_count = (
        Sound.objects.filter(processing_state="PE")
...
def monitor_analysis(request):
    ...
    consolidated_ok = SoundAnalysis.objects.filter(
        analyzer=settings.CONSOLIDATED_ANALYZER_NAME, analysis_status="OK"
    ).count()
    consolidated_sk = SoundAnalysis.objects.filter(
        analyzer=settings.CONSOLIDATED_ANALYZER_NAME, analysis_status="SK"
    ).count()
    consolidated_fa = SoundAnalysis.objects.filter(
        analyzer=settings.CONSOLIDATED_ANALYZER_NAME, analysis_status="FA"
    ).count()
    consolidated_qu = SoundAnalysis.objects.filter(
...
def process_sounds(request):
    ...
    if processing_status:
        ...
        if sounds_to_process:
            sounds_to_process = sounds_to_process.exclude(processing_ongoing_state="PR").exclude(
                processing_ongoing_state="QU"
            )
...
def moderator_stats_ajax(request):
    ...
    time_span = timezone.now() - datetime.timedelta((6 * 365) // 12)
    tickets_mod = (
        tickets.models.Ticket.objects.filter(assignee_id=user_id, status=TICKET_STATUS_CLOSED, created__gt=time_span)
        .extra(select={"day": "date(modified)"})
        .values("day")
        .order_by()
        .annotate(Count("id"))
    )
