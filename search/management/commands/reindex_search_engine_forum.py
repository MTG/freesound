...
class Command(BaseCommand):
    help = (
        "Index all moderated forum posts in the search engine. If there are remaining forum posts in the search"
    ...
    def handle(self, *args, **options):
        ...
        all_posts = Post.objects.select_related("thread", "author", "thread__author", "thread__forum").filter(
            moderation_state="OK"
        )
        ...
        for i in range(0, num_posts, slice_size):
            ...
            console_logger.info(
                f"Added {n_posts_indexed_correctly}/{num_posts} posts. Elapsed: {elapsed}, Remaining: {remaining}"
            )
