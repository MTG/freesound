    ...
    def setUp(self):
        ...
        results.docs = [
            {'group_docs': [{'id': sound_id}],
             'id': sound_id,
             'n_more_in_group': 0,
             'group_name': str(sound_id)} for sound_id in sound_ids
        ]
        paginator = SearchResultsPaginator(results, self.NUM_RESULTS)
        page = paginator.page(1)  # Get first page
        self.perform_search_engine_query_response = (results, paginator)
