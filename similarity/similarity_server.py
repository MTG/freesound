...
def server_interface(resource):
    return {
        ...
        'get_sounds_descriptors':
            resource.get_sounds_descriptors,    # sound_ids, descritor_names (optional), normalization (optional)
...
class SimilarityServer(resource.Resource):
    ...
    def get_sounds_descriptors(
        self, request, sound_ids, descriptor_names=None, normalization=[0], only_leaf_descriptors=[0]
    ):
        kwargs = dict()
        if descriptor_names:
    ...
    def nnsearch(self, request, sound_id=None, num_results=None, preset=None, offset=[0]):
        ...
        return json.dumps(
            self.gaia.search_dataset(sound_id[0], num_results[0], preset_name=preset[0], offset=offset[0])
        )
    ...
    def api_search(
        ...
        if not target and not filter:
            if target_type:
                if target_type[0] != 'file':
                    return json.dumps({
                        'error': True,
                        'result': 'At least \'target\' or \'filter\' should be specified.',
        ...
        if target and not target_type:
            return json.dumps({
                'error': True,
                'result': 'Parameter \'target_type\' should be specified.',
                'status_code': BAD_REQUEST_CODE
            })
        if target_type:
        ...
        if target or target_type == 'file':
            if target_type == 'sound_id':
                ...
                try:
                    ...
                    if not isinstance(target, dict):
                        if isinstance(target, str):
                            ...
                            return json.dumps({
                                'error': True,
                                'result': 'Invalid descriptor values for target.',
                                'status_code': BAD_REQUEST_CODE
                            })
                    if not target.items():
                ...
                data = request.content.getvalue().split('&')[
                    0]    # If more than one file attached, just get the first one
                if not data:
                    return json.dumps({
                        'error': True,
                        'result': 'You specified \'file\' as target file but attached no analysis file.',
                        'status_code': BAD_REQUEST_CODE
                    })
                try:
                    ...
                    return json.dumps({
                        'error': True,
                        'result': 'Analysis file could not be parsed.',
                        'status_code': BAD_REQUEST_CODE
                    })
            else:
        ...
        if target_type == 'descriptor_values' and target:
            metric_descriptor_names = parse_metric_descriptors(
                ','.join(target.keys()), self.gaia.descriptor_names['fixed-length']
            )
        else:
            metric_descriptor_names = False    # For the moment metric_descriptor_names can only be set by us

        return json.dumps(
            self.gaia.api_search(
                target_type, target, filter, preset, metric_descriptor_names, num_results, offset, in_ids
            )
        )
