...
class SoundAdmin(DjangoObjectActions, admin.ModelAdmin):
    fieldsets = (
        (None, {
            'fields': ('user', 'num_downloads')
        }),
        ('Filenames', {
            'fields': ('base_filename_slug',)
        }),
        ('User defined fields', {
            'fields': ('description', 'license', 'original_filename', 'sources', 'pack')
        }),
        (
            'File properties', {
                'fields': (
                    'md5', 'type', 'duration', 'bitrate', 'bitdepth', 'samplerate', 'filesize', 'channels',
                    'date_recorded'
                )
            }
        ),
        ('Moderation', {
            'fields': ('moderation_state', 'moderation_date', 'has_bad_description', 'is_explicit')
        }),
        (
            'Processing', {
                'fields': (
                    'processing_state', 'processing_date', 'processing_ongoing_state', 'processing_log',
                    'similarity_state'
                )
            }
        ),
...
class FlagAdmin(admin.ModelAdmin):
    ...
    def sound_link(self, obj):
        return mark_safe(
            '<a href="{}" target="_blank">{}</a>'.format(
                reverse('short-sound-link', args=[obj.sound_id]), truncatechars(obj.sound.base_filename_slug, 50)
            )
        )
...
class SoundAnalysisAdmin(DjangoObjectActions, admin.ModelAdmin):
    list_display = (
        'analyzer', 'sound_id', 'analysis_status', 'last_sent_to_queue', 'last_analyzer_finished',
    ...
    def re_run_analysis(self, request, queryset_or_object):
        if isinstance(queryset_or_object, SoundAnalysis):
            ...
            messages.add_message(
                request, messages.INFO, 'Sound {} was sent to re-analyze with analyzer {}.'.format(
