...
class Command(LoggingBaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        sound_analysis_objects_ids = SoundAnalysis.objects.filter(
        ...
        for descriptor_name in descriptor_names:
            ...
            descriptor_values = list(
                SoundAnalysis.objects.filter(id__in=sound_analysis_objects_ids).values_list(
                    f"analysis_data__{descriptor_name}", flat=True
                )
            )
            ...
            console_logger.info(
                f"Exported {len(descriptor_values)} values for descriptor {descriptor_name} to {output_file_path}"
            )
