        ...
        try:
            ...
            if result:
                workers_logger.info("Finished analysis of sound (%s)" % json.dumps(
                    {'sound_id': sound_id, 'result': 'success', 'work_time': time.time() - starttime}))
            else:
                workers_logger.info("Finished analysis of sound (%s)" % json.dumps(
                    {'sound_id': sound_id, 'result': 'failure', 'work_time': time.time() - starttime}))

        except WorkerException as e:
            workers_logger.error("WorkerException while analyzing sound (%s)" % json.dumps(
                {'sound_id': sound_id, 'error': str(e), 'work_time': time.time() - starttime}))
