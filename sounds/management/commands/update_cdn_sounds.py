...
class Command(LoggingBaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            dest="filepath",
            type=str,
            help="Path to JSON file with sounds map. If using this option, no new sounds will be copied to the CDN but only the local map in cache will be updated",
        )
        parser.add_argument(
            "-k",
            "--keypath",
                    ...
                    for count, sound in enumerate(ss):
                        ...
                        if not os.path.exists(src_sound_path):
                            console_logger.error(
                                f"[{count + 1}/{total}] Sound with id {sound_id} does not exist, skipping"
                            )
                            num_failed += 1
                            continue
                        ...
                        if sound_exists:
                            c.run(
                                f"rm {os.path.join(cdn_symlinks_dir, folder_id, f'{sound_id}-*')}", hide=True, warn=True
                            )
