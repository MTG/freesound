...
def front_page(request):
    ...
    if popular_searches is not None:
        popular_searches = [
            (query_terms, "{0}?q={1}".format(reverse("sounds-search"), query_terms))
            for query_terms in popular_searches
        ]

    current_forum_threads = (
        Thread.objects.filter(
            first_post__moderation_state="OK", last_post__moderation_state="OK"
        )
    ...
    if using_beastwhoosh(request):
        ...
        if top_donor_data:
            top_donor_username = sorted(
                top_donor_data.items(), key=lambda x: x[1], reverse=True
            )[0][0]
...
def sound(request, username, sound_id):
    try:
        sound = (
            Sound.objects.prefetch_related("tags__tag")
    ...
    if request.user.is_authenticated:
        ...
        if sound.user in users_following:
            is_following = True
    is_explicit = sound.is_explicit and (
...
def after_download_modal(request):
    ...
    sound_name = request.GET.get(
        "sound_name", "this sound"
    )  # Gets some data sent by the client
    ...
    if DonationsModalSettings.get_donation_modal_settings().enabled:
        ...
        modal_shown_timestamps = [
            item for item in modal_shown_timestamps if item > (time.time() - 24 * 3600)
        ]

        if should_suggest_donation(request.user, len(modal_shown_timestamps)):
            web_logger.info(
                "Showing after download donate modal (%s)"
...
def sound_download(request, username, sound_id):
    if not request.user.is_authenticated:
        return HttpResponseRedirect(
            "%s?next=%s"
            % (reverse("accounts-login"), reverse("sound", args=[username, sound_id]))
        )
    sound = get_object_or_404(
        Sound, id=sound_id, moderation_state="OK", processing_state="OK"
    )
    ...
    if "HTTP_RANGE" not in request.META:
        ...
        if cache.get(cache_key, None) is None:
            Download.objects.create(
                user=request.user, sound=sound, license_id=sound.license_id
            )
            sound.invalidate_template_caches()
            cache.set(
                cache_key, True, 60 * 5
            )  # Don't save downloads for the same user/sound in 5 minutes
...
def pack_download(request, username, pack_id):
    if not request.user.is_authenticated:
        return HttpResponseRedirect(
            "%s?next=%s"
            % (reverse("accounts-login"), reverse("pack", args=[username, pack_id]))
        )
    ...
    if "HTTP_RANGE" not in request.META:
        ...
        if cache.get(cache_key, None) is None:
            ...
            for sound in pack.sounds.all():
                pds.append(
                    PackDownloadSound(
                        sound=sound, license_id=sound.license_id, pack_download=pd
                    )
                )
...
def sound_edit(request, username, sound_id):
    ...
    def update_sound_tickets(sound, text):
        tickets = Ticket.objects.filter(sound_id=sound.id).exclude(
            status=TICKET_STATUS_CLOSED
        )
        for ticket in tickets:
            tc = TicketComment(
                sender=request.user, ticket=ticket, moderator_only=False, text=text
            )
            tc.save()
            ticket.send_notification_emails(
                ticket.NOTIFICATION_UPDATED, ticket.MODERATOR_ONLY
            )

    if is_selected("description"):
        description_form = SoundDescriptionForm(
            request.POST, prefix="description", explicit_disable=sound.is_explicit
        )
        ...
        if description_form.is_valid():
            ...
            update_sound_tickets(
                sound,
                "%s updated the sound description and/or tags." % request.user.username,
            )
            return HttpResponseRedirect(sound.get_absolute_url())
    else:
        tags = " ".join(
        ...
        description_form = SoundDescriptionForm(
            ...
            initial=dict(
                tags=tags, description=sound.description, name=sound.original_filename
            ),
        )
        ...
        if pack_form.is_valid():
            ...
            if data["new_pack"]:
                (pack, created) = Pack.objects.get_or_create(
                    user=sound.user, name=data["new_pack"]
                )
                if sound.pack:
                    affected_packs.append(
                        sound.pack
                    )  # Append previous sound pack if exists
                sound.pack = pack
    ...
    if is_selected("geotag"):
        ...
        if sound.geotag:
            geotag_form = GeotaggingForm(
                prefix="geotag",
                initial=dict(
                    lat=sound.geotag.lat, lon=sound.geotag.lon, zoom=sound.geotag.zoom
                ),
            )
        else:
...
def pack_delete(request, username, pack_id):
    ...
    if encrypted_string is not None:
        ...
        if abs(time.time() - link_generated_time) < 10:
            web_logger.info(
                "User %s requested to delete pack %s" % (request.user.username, pack_id)
            )
...
def pack(request, username, pack_id):
    ...
    tvars = {"pack": pack, "num_sounds_ok": num_sounds_ok, "pack_sounds": pack_sounds}
...
def packs_for_user(request, username):
    ...
    qs = (
        Pack.objects.select_related()
        .filter(user=user, num_sounds__gt=0)
        .exclude(is_deleted=True)
        .order_by(order)
    )
...
def for_user(request, username):
    ...
    paginator = paginate(
        request,
        Sound.public.only("id").filter(user=sound_user),
        settings.SOUNDS_PER_PAGE,
    )
    sound_ids = [sound_obj.id for sound_obj in paginator["page"]]
    user_sounds = Sound.objects.ordered_ids(sound_ids)

    tvars = {"sound_user": sound_user, "user_sounds": user_sounds}
    tvars.update(paginator)
...
def delete(request, username, sound_id):
    ...
    if request.method == "POST":
        ...
        if not form.is_valid():
            ...
            web_logger.info(
                "User %s requested to delete sound %s"
    ...
    tvars = {"error_message": error_message, "delete_form": form, "sound": sound}
    ...
    if obj_id:
        try:
            ...
            return HttpResponsePermanentRedirect(
                reverse(url_name, args=[obj.user.username, obj_id])
            )
        except ValueError:
            raise Http404
    else:
...
def pack_downloaders(request, username, pack_id):
    ...
    # Retrieve all users that downloaded a sound
    qs = PackDownload.objects.filter(pack_id=pack_id).select_related(
        "user", "user__profile"
    )
