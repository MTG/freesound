...
def main():
    ...
    while True:
        ...
        if not processes:
            ...
            # Absolute CPU values
            stats = {f"gunicorn_cpu_percent_{pid}": cpu for pid, cpu in pid_to_percent.items()}
            ...
            stats.update({f"gunicorn_cpu_hist_{range_key}": count for range_key, count in hist.items()})
