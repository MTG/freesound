...
def get_clusters_for_query(sqp, compute_if_not_in_cache=True):
    ...
    if results is None and compute_if_not_in_cache:
        ...
        if sound_ids:
            ...
            async_task_result = cluster_sounds.apply_async(
                kwargs={
                    "cache_key": cache_key,
                    "sound_ids": sound_ids,
            ...
            if results["clusters"] is not None:
                ...
                partition = {
                    sound_id: cluster_id for cluster_id, cluster in enumerate(clusters) for sound_id in cluster
                }
                ...
                cluster_most_occurring_tags = [
                    [
                        tag
                        for tag, _ in Counter(cluster_tags[cluster_id]).most_common(settings.NUM_TAGS_SHOWN_PER_CLUSTER)
                    ]
                    if cluster_tags[cluster_id]
                    else []
                    for cluster_id in range(len(clusters))
                ]
                most_occurring_tags_formatted = [
                    " ".join(sorted(most_occurring_tags)) for most_occurring_tags in cluster_most_occurring_tags
                ]
...
def get_clustering_data_for_graph_display(sqp, initial_graph):
    ...
    if graph is None:
        ...
        if len(sqp.non_option_filters):
            ...
            graph["nodes"] = []
            graph["links"] = []
            sound_ids_filtered = get_sound_ids_from_search_engine_query(
                query_params, num_sounds=settings.MAX_RESULTS_FOR_CLUSTERING, current_page=1
            )
            for node in nodes:
    ...
    if num_sounds_per_cluster is None:
        if clusters:
            ...
            if len(sqp.non_option_filters):
                ...
                clusters = [
                    [sound_id for sound_id in cluster if int(sound_id) in sound_ids_filtered] for cluster in clusters
                ]
