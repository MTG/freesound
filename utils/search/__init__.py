...
def get_search_engine(
    ...
    Returns:
        search engine backend class instance
    """
...
class SearchResults:
    def __init__(
        ...
        highlighting: dict[str, dict] | None = None,
        q_time: int = -1,
    ):
        """
        ...
        Args:
            docs: list of dictionaries with the results of the query. Each dict object includes information
            ...
            start: offset of the search results query (Eg: return matched documents starting at 15)
            num_rows: number of results per "page"
            num_found: total number of matches found
            non_grouped_number_of_results: total number of non-grouped matches found (it will be
                the same as num_found for queries which did not group results)
            facets: data structure including information about the facets
            ...
            highlighting: data structure containing "highlighted" contents of the search results (if
            ...
            q_time: time that it took to execute the query in the backend, in ms
        """
...
class SearchEngineBase:
    ...
    def add_sounds_to_index(
        self, sound_objects: list[sounds.models.Sound], update: bool = False, include_similarity_vectors: bool = False
    ):
        """Indexes the provided sound objects in the search index

        Args:
            sound_objects: Sound objects of the sounds to index
            update: Whether to perform an update of the existing documents in the index or to
                completely replace them. An update is useful so that fields not included in the document are
                not removed from the index.
            include_similarity_vectors: Whether to include similarity vectors in the index.
        """
    ...
    def remove_sounds_from_index(self, sound_objects_or_ids: list[sounds.models.Sound] | list[int]):
        """Removes sounds from the search index

        Args:
            sound_objects_or_ids: Sound objects or sound IDs to remove
        """
        raise NotImplementedError
    ...
    def sound_exists_in_index(self, sound_object_or_id: sounds.models.Sound | int) -> bool:
        """Check if a sound is indexed in the search engine

        Args:
            sound_object_or_id: Sound object or sound ID to check if indexed

        Returns:
            whether the sound is indexed in the search engine
        """
        raise NotImplementedError

    def get_all_sound_ids_from_index(self) -> list[int]:
        """Return a list of all sound IDs indexed in the search engine

        Returns:
            list of all sound IDs indexed in the search engine
        """
        raise NotImplementedError
    ...
    def search_sounds(
        self,
        textual_query: str = "",
        query_fields: list[str] | dict[str, int] | None = None,
        query_filter: str = "",
        field_list: list[str] | None = None,
        offset: int = 0,
        current_page: int | None = None,
        num_sounds: int = settings.SOUNDS_PER_PAGE,
        sort: str = settings.SEARCH_SOUNDS_SORT_OPTION_AUTOMATIC,
        group_by_pack: bool = False,
        num_sounds_per_pack_group: int = 1,
        facets: dict[str, dict] | None = None,
        only_sounds_with_pack: bool = False,
        only_sounds_within_ids: list[int] | bool = False,
        group_counts_as_one_in_facets: bool = False,
        similar_to: int | list[float] | None = None,
        similar_to_min_similarity: float = settings.SIMILARITY_MIN_THRESHOLD,
        similar_to_similarity_space: str = settings.SIMILARITY_SPACE_DEFAULT,
    ) -> SearchResults:
        """Search for sounds that match specific criteria and return them in a SearchResults object

        Args:
            textual_query: the textual query
            query_fields: a list of the fields that should be matched when
            ...
            query_filter: filter expression following lucene filter syntax
            field_list: list of fields to return by the search engine. Typically we're only interested
            ...
            offset: offset for the returned results
            current_page: alternative way to set offset using page numbers. Using current_page will
                set offset like offset=current_page*num_sounds
            num_sounds: number of sounds to return
            sort: sorting criteria. should be one of settings.SEARCH_SOUNDS_SORT_OPTIONS_WEB
            group_by_pack: whether the search results should be grouped by sound pack. When grouped
            ...
            num_sounds_per_pack_group: number of sounds to return per pack group (minimum one)
            facets: information about facets to be returned. Can be None if no faceting
            ...
            group_counts_as_one_in_facets: whether only one result from a group should be counted
                ...
                counted)
            similar_to: sound ID or similarity vector to be used as target for similarity
            ...
            similar_to_min_similarity: min similarity score to consider a sound as similar.
            similar_to_similarity_space: similarity space from which to select similarity vectors for
        ...
        Returns:
            SearchResults object containing the results of the query
        """
        raise NotImplementedError

    def get_random_sound_id(self) -> int:
        ...
        Returns:
            the ID of the selected random sound (or 0 if there were errors)
        """
        raise NotImplementedError

    def get_num_sim_vectors_indexed_per_similarity_space(self) -> dict:
        ...
        Returns:
            dictionary with the number of similarity vectors and number of sounds indexed per similarity space.
                E.g.: {'freesound_classic': {'num_sounds': 0, 'num_vectors': 0},
                       'laion_clap': {'num_sounds': 15876, 'num_vectors': 25448}}
        """
        raise NotImplementedError

    def get_all_sim_vector_document_ids_per_similarity_space(self) -> dict:
        ...
        Returns:
            dictionary with a list of Solr document IDs per similarity space.
    ...
    def add_forum_posts_to_index(self, forum_post_objects: list[forum.models.Post]):
        """Indexes the provided forum post objects in the search index

        Args:
            forum_post_objects: Post objects of the forum posts to index
        """
        raise NotImplementedError

    def remove_forum_posts_from_index(self, forum_post_objects_or_ids: list[forum.models.Post] | list[int]):
        """Removes forum posts from the search index

        Args:
            forum_post_objects_or_ids: Post objects or post IDs to remove
        """
        raise NotImplementedError
    ...
    def forum_post_exists_in_index(self, forum_post_object_or_id: forum.models.Post | int) -> bool:
        """Check if a sound is indexed in the search engine

        Args:
            forum_post_object_or_id: Post object or post ID to check if indexed

        Returns:
            whether the post is indexed in the search engine
        """
        raise NotImplementedError
    ...
    def search_forum_posts(
        ...
        sort: str | None = None,
        current_page: int | None = None,
        num_posts: int = settings.FORUM_POSTS_PER_PAGE,
        ...
        Args:
            ...
            query_filter: filter expression following lucene filter syntax
            offset: offset for the returned results
            sort: sorting criteria. should be one of settings.SEARCH_FORUM_SORT_OPTIONS_WEB
            current_page: alternative way to set offset using page numbers. Using current_page will
                set offset like offset=current_page*num_sounds
            num_posts: number of forum posts to return
            group_by_thread: whether the search results should be grouped by forum post thread. When
    ...
    def get_user_tags(self, username) -> list[tuple[str, int]]:
        ...
        Returns:
            List of tuples with the tags and counts of the tags used by the user.
                Eg: [('cat', 1), ('echo', 1), ('forest', 1)]
        """
        raise NotImplementedError

    def get_pack_tags(self, username, pack_name) -> list[tuple[str, int]]:
        ...
        Returns:
            List of tuples with the tags and counts of the tags in the pack.
                Eg: [('cat', 1), ('echo', 1), ('forest', 1)]
        """
        raise NotImplementedError
