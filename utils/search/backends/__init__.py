...
from sounds.models import SoundAnalysis, Sound
from utils.search import (
    SOLR_DYNAMIC_FIELDS_SUFFIX_MAP,
    SOLR_DOC_CONTENT_TYPES,
    get_solr_dense_vector_search_field_name,
...
class SearchEngineBase:
    ...
    @property
    def sounds_index(self) -> pysolr.Solr:
        """Return the sounds index object"""
        raise NotImplementedError

    @property
    def forum_index(self) -> pysolr.Solr:
        """Return the forum index object"""
        raise NotImplementedError
    ...
    def add_sounds_to_index(self, sound_objects, fields_to_include=[], update=False):
        ...
        documents = [
            self.convert_sound_to_search_engine_document(s, fields_to_include=fields_to_include) for s in sound_objects
        ]
        ...
        try:
            self.sounds_index.add(documents)
    ...
    def convert_sound_to_search_engine_document(self, sound, fields_to_include=[]):
        ...
        if getattr(sound, "pack_id"):
            ...
            document["grouping_pack"] = (
                str(getattr(sound, "pack_id")) + "_" + remove_control_chars(getattr(sound, "pack_name"))
            )
        else:
    ...
    def add_similarity_vectors_to_documents(self, sound_objects, documents):
        ...
        for analyzer_name, config_options in settings.SEARCH_ENGINE_SIMILARITY_ANALYZERS.items():
            ...
            if vector_field_name is None:
                ...
                continue
            for sa in SoundAnalysis.objects.filter(
                ...
                if data is not None:
                    if data.get(config_options["vector_property_name"], None) is not None:
                        ...
                        sim_vector_document_data = {
                            ...
                            "timestamp_start": 0,
                            # This will be used in the future if analyzers generate multiple sound vectors
                            "timestamp_end": -1,
                            # This will be used in the future if analyzers generate multiple sound vectors
                        ...
                        if getattr(sound_objects_dict[sa.sound_id], "pack_id"):
                            sim_vector_document_data["grouping_pack_child"] = (
                                str(getattr(sound_objects_dict[sa.sound_id], "pack_id"))
                                + "_"
                                + remove_control_chars(getattr(sound_objects_dict[sa.sound_id], "pack_name"))
                            )
                        else:
                            sim_vector_document_data["grouping_pack_child"] = str(
                                getattr(sound_objects_dict[sa.sound_id], "id")
                            )
    ...
    def remove_sounds_from_index(self, sound_objects_or_ids):
        ...
        try:
            ...
            self.sounds_index.delete(id=sound_ids)
        except pysolr.SolrError as e:
            raise SearchEngineException(e)
    ...
    def add_solr_suffix_to_dynamic_fieldnames_in_filter(self, query_filter):
        ...
        fields which need to end with a specific suffix that SOLR uses to learn about the type of the field and how it
        should treat it.
        """
    ...
    def search_sounds(
        ...
        if similar_to is None:
            ...
            if isinstance(query_fields, list):
                ...
                query_fields = [
                    (self.get_solr_fieldname(field_name), weight)
            ...
            if similar_to_analyzer in settings.SEARCH_ENGINE_SIMILARITY_ANALYZERS:
                ...
                if vector is not None and vector_field_name is not None:
                    max_similar_sounds = similar_to_max_num_sounds  # Max number of results for similarity search. Filters are applied before the similarity search, so this number will usually be the total number of results (unless filters are more restrictive)
        ...
        if similar_to is not None:
            ...
            query_filter_modified = [
                ...
                f"analyzer:{similar_to_analyzer}",
            ]  # Add basic filter to only get similarity vectors from selected analyzer and from child documents (this is because root documents can also have sim vectors)
            ...
            try:
                ...
                top_similar_sounds_as_filter = top_similar_sounds_as_filter.replace(
                    f"topK={similar_to_max_num_sounds}", f"topK={similar_to_max_num_sounds + 1}"
                )
            except TypeError:
        ...
        # Configure grouping
        if group_by_pack:
            query.set_group_field(
                group_field="grouping_pack" if not similar_to else "grouping_pack_child"
            )  # We name the fields differently to avoid solr conflicts with matches of both child and parent docs
            query.set_group_options(
                group_func=None,
                group_query=None,
                group_rows=10,  # TODO: if limit is lower than rows and start=0, this should probably be equal to limit
                group_start=0,
                group_limit=num_sounds_per_pack_group,
                # This is the number of documents that will be returned for each group.
                group_offset=0,
                group_sort=None,
        ...
        try:
            ...
            results = self.sounds_index.search(
            ...
            results.facets = {
                self.get_original_fieldname(facet_name): data for facet_name, data in results.facets.items()
            }  # Why so complicated?
    ...
    def get_random_sound_id(self):
        ...
        try:
            response = self.sounds_index.search(search_handler="select", **self.force_sounds(query.as_kwargs()))
            docs = response.docs
            if docs:
    ...
    def get_num_sim_vectors_indexed_per_analyzer(self):
        ...
        for analyzer_name in settings.SEARCH_ENGINE_SIMILARITY_ANALYZERS.keys():
            ...
            query.set_group_options()
            try:
                response = self.sounds_index.search(search_handler="select", **query.as_kwargs())
    ...
    def add_forum_posts_to_index(self, forum_post_objects):
        ...
        try:
            self.forum_index.add(documents)
        except pysolr.SolrError as e:
            raise SearchEngineException(e)
    ...
    def remove_forum_posts_from_index(self, forum_post_objects_or_ids):
        ...
        try:
            for post_object_or_id in forum_post_objects_or_ids:
                ...
                self.forum_index.delete(id=str(post_id))
        except pysolr.SolrError as e:
            raise SearchEngineException(e)
    ...
    def search_forum_posts(
        ...
        # We do it in this way to conform to SearchEngine.search_sounds definition which must return SearchResults
        try:
            results = self.forum_index.search(**query.as_kwargs())
            return SearchResults(
                docs=results.docs,
                num_found=results.num_found,
                start=results.start,
    ...
    def get_user_tags(self, username):
        ...
        try:
            results = self.sounds_index.search(**self.force_sounds(query.as_kwargs()))
