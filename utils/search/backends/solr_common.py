...
class SolrQuery:
    ...
    def set_query_options(self, start=None, rows=None, sort=None, filter_query=None, field_list=None):
        ...
        if "dist" in self.params["sort"]:
            if field_list is None:
                field_list = []
            field_list.append(sort[0].replace(" desc", "").replace(" asc", ""))
    ...
    def set_facet_options_default(
        self, limit=None, offset=None, prefix=None, sort=None, mincount=None, count_missing=None, enum_cache_mindf=None
    ):
    ...
    def set_highlighting_options(
        self, field, snippets=None, fragment_size=None, merge_contiguous=None, alternate_field=None, pre=None, post=None
    ):
    ...
    def __init__(self, response, next_page_query=None):
        ...
        if "grouped" in response:
            ...
            self.docs = [
                ...
                if group["groupValue"] is not None
            ]
            ...
            for doc in response["response"]["docs"]:
                ...
                group_docs = (
                    response["expanded"][doc[collapse_field]]["docs"]
                    if collapse_field in doc and doc[collapse_field] in response["expanded"]
                    else []
                )
                group_docs = [doc] + group_docs  # Add the original document to the group docs list
                n_more_in_group = (
                    response["expanded"][doc[collapse_field]]["numFound"]
                    if collapse_field in doc and doc[collapse_field] in response["expanded"]
                    else 0
                )
                self.docs.append(
            ...
            self.num_rows = len(self.docs)
            self.num_found = response[
                "response"
            ][
                "numFound"
            ]  # This corresponds to the total number of groups (including sounds without group being counted as one group because of nullPolicy=expand)
            self.non_grouped_number_of_results = -1.0  # When using the collapse and expand query parser, we don't know the number of uncollapsed results, this will be obtained later making a second query
        else:
        ...
        if settings.DEBUG or (
            settings.SEARCH_LOG_SLOW_QUERIES_MS_THRESHOLD > -1
            and self.q_time > settings.SEARCH_LOG_SLOW_QUERIES_MS_THRESHOLD
        ):
            ...
            if (
                ...
                search_logger.info(
                    "SOLR slow query detected (%s)"
                    % json.dumps({"q_time": self.q_time, "num_results": self.num_found, "url": solr_query_url})
                )
