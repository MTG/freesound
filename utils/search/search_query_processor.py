...
class SearchQueryProcessor:
    ...
    option_definitions = [
        ...
        (
            "compute_clusters",
            SearchOptionBool,
            dict(query_param_name="cc", label="Cluster results by sound similarity"),
        ),
        (
            "cluster_id",
            SearchOptionInt,
        ...
        ("field_weights", SearchOptionFieldWeights, dict(query_param_name="w")),
        (
            "include_audio_problems",
            SearchOptionBoolFilterInverted,
            dict(
    ...
    def __init__(self, request, facets=None):
        ...
        if facets is None:
            self.facets = (
                settings.SEARCH_SOUNDS_DEFAULT_FACETS.copy()
            )  # NOTE: not sure if .copy() is needed here to avoid mutating original setting
        else:
            self.facets = facets
        ...
        values_to_update = {}
        for field_name in [
            self.options["is_remix"].search_engine_field_name,
            self.options["is_geotagged"].search_engine_field_name,
        ]:
            for node in self.f_parsed:
        ...
        if values_to_update:
            ...
            if self.options["is_remix"].search_engine_field_name in values_to_update:
                self.request.GET[self.options["is_remix"].query_param_name] = (
                    "1" if values_to_update[self.options["is_remix"].search_engine_field_name] else "0"
                )
            if self.options["is_geotagged"].search_engine_field_name in values_to_update:
                self.request.GET[self.options["is_geotagged"].query_param_name] = (
                    "1" if values_to_update[self.options["is_geotagged"].search_engine_field_name] else "0"
                )
            if self.options["duration"].search_engine_field_name in values_to_update:
                self.request.GET[self.options["duration"].query_param_min] = values_to_update[
                    self.options["duration"].search_engine_field_name
                ][0]
                self.request.GET[self.options["duration"].query_param_max] = values_to_update[
                    self.options["duration"].search_engine_field_name
                ][1]
        ...
        search_engine_field_names_used_in_options = [
            option.search_engine_field_name
            for option in self.options.values()
            if hasattr(option, "search_engine_field_name")
        ]
        for node in self.f_parsed:
            if type(node) == luqum.tree.SearchField:
    ...
    def get_num_active_filters(
        ...
        return len(
            self.get_active_filters(
                include_filters_from_options=include_filters_from_options,
                include_non_option_filters=include_non_option_filters,
                include_filters_from_facets=include_filters_from_facets,
    ...
    def get_filters_data_to_display_in_search_results_page(self):
        ...
        if cluster_id > -1:
            ...
            if (
                cluster_results is not None
                and cluster_results["clusters"] is not None
                and cluster_id in cluster_results["cluster_ids"]
            ):
                cluster_number = cluster_results["cluster_ids"].index(cluster_id) + 1
                filters_data.append(
                    ["cluster", f"#{cluster_number}", self.get_url().replace(f"cid={cluster_id}", "cid=-1")]
                )
    ...
    def get_filter_string_for_search_engine(
        self,
        include_filters_from_options=True,
        include_non_option_filters=True,
        include_filters_from_facets=True,
    ...
    def get_filter_string_for_url(self, extra_filters=None, ignore_filters=None):
        ...
        return self.get_filter_string_for_search_engine(
            include_filters_from_options=False, extra_filters=extra_filters, ignore_filters=ignore_filters
        )
    ...
    def get_clustering_data_cache_key(self, include_filters_from_facets=False):
        ...
        key = (
            f"cluster-results-{self.get_option_value_to_apply('query')}-"
    ...
    def as_query_params(self, exclude_facet_filters=False):
        ...
        if self.get_option_value_to_apply("display_as_packs"):
            ...
            num_sounds = (
                ...
                if not self.get_option_value_to_apply("grid_mode")
        ...
        return dict(
            ...
            query_filter=self.get_filter_string_for_search_engine(
                include_filters_from_facets=not exclude_facet_filters
            ),
            ...
            if not self.get_option_value_to_apply("map_mode")
            else settings.MAX_SEARCH_RESULTS_IN_MAP_DISPLAY,
            sort=self.get_option_value_to_apply("sort_by"),
            group_by_pack=self.get_option_value_to_apply("group_by_pack")
            or self.get_option_value_to_apply("display_as_packs"),
