...
class SearchQueryProcessorTests(TestCase):
    default_expected_params = {
        ...
        | settings.SEARCH_SOUNDS_BETA_FACETS,  # Combine all facets because we normally test with superuser,
        "field_list": ["id", "score"],
        "group_by_pack": True,
    ...
    def test_search_query_processor_as_query_params_and_make_url(self, fake_get_ids_in_cluster):
        ...
        self.assertExpectedParams(
            sqp.as_query_params(), {"textual_query": "test", "sort": settings.SEARCH_SOUNDS_SORT_OPTION_AUTOMATIC}
        )
        self.assertGetUrlAsExpected(sqp, url)
        ...
        sqp, url = self.run_fake_search_query_processor(
            params={"si_tags": "1", "si_description": "1", "si_sound_id": "0"}
        )
        self.assertExpectedParams(
            sqp.as_query_params(),
            {
                "query_fields": {
                    settings.SEARCH_SOUNDS_FIELD_DESCRIPTION: settings.SEARCH_SOUNDS_DEFAULT_FIELD_WEIGHTS[
                        settings.SEARCH_SOUNDS_FIELD_DESCRIPTION
                    ],
                    settings.SEARCH_SOUNDS_FIELD_TAGS: settings.SEARCH_SOUNDS_DEFAULT_FIELD_WEIGHTS[
                        settings.SEARCH_SOUNDS_FIELD_TAGS
                    ],
                }
            },
        )
        self.assertGetUrlAsExpected(
            sqp, url.replace("si_sound_id=0", "")
        )  # Here we remove a_soundid from the expected URL because sqp.get_url() will exclude it as value is not '1'

        # With custom field weights specified
        sqp, url = self.run_fake_search_query_processor(
            params={"w": f"{settings.SEARCH_SOUNDS_FIELD_DESCRIPTION}:2,{settings.SEARCH_SOUNDS_FIELD_ID}:1"}
        )
        self.assertExpectedParams(
            sqp.as_query_params(),
            {"query_fields": {settings.SEARCH_SOUNDS_FIELD_DESCRIPTION: 2, settings.SEARCH_SOUNDS_FIELD_ID: 1}},
        )
        self.assertGetUrlAsExpected(sqp, url)
        ...
        sqp, url = self.run_fake_search_query_processor(
            params={
                "si_sound_id": "1",
                "w": f"{settings.SEARCH_SOUNDS_FIELD_DESCRIPTION}:2,{settings.SEARCH_SOUNDS_FIELD_ID}:1",
            }
        )
        self.assertExpectedParams(sqp.as_query_params(), {"query_fields": {settings.SEARCH_SOUNDS_FIELD_ID: 1}})
        self.assertGetUrlAsExpected(sqp, url)
        ...
        self.assertExpectedParams(
            sqp.as_query_params(), {"query_filter": ""}
        )  # If geotagged option is 0, no filter should be added
        ...
        self.assertExpectedParams(
            sqp.as_query_params(), {"query_filter": ""}
        )  # If remix option is 0, no filter should be added
        ...
        self.assertExpectedParams(
            sqp.as_query_params(),
            {"group_by_pack": True, "only_sounds_with_pack": True, "num_sounds_per_pack_group": 3},
        )
        self.assertGetUrlAsExpected(sqp, url)
        sqp, url = self.run_fake_search_query_processor(
            params={"dp": "1", "g": "0"}
        )  # When display packs is enabled, always group by pack
        self.assertExpectedParams(
            sqp.as_query_params(),
            {"group_by_pack": True, "only_sounds_with_pack": True, "num_sounds_per_pack_group": 3},
        )
        ...
        sqp, url = self.run_fake_search_query_processor(
            params={"cm": "1", "dp": "1"}
        )  # In display pack mode, number of sounds stays the same
        self.assertExpectedParams(
            sqp.as_query_params(),
            {"num_sounds": settings.SOUNDS_PER_PAGE, "only_sounds_with_pack": True, "num_sounds_per_pack_group": 3},
        )
        self.assertGetUrlAsExpected(sqp, url)
        ...
        sqp, url = self.run_fake_search_query_processor(
            params={"mm": "1", "page": "3"}
        )  # Page number in map mode is always 1
        self.assertExpectedParams(
            sqp.as_query_params(),
            {
                "group_by_pack": False,
        ...
        self.assertExpectedParams(
            sqp.as_query_params(), {"query_filter": 'pack_grouping:"19894_Clutter"', "group_by_pack": False}
        )
        self.assertGetUrlAsExpected(sqp, url)
    ...
    def test_search_query_processor_contains_active_advanced_search_options(self):
        ...
        self.assertEqual(
            sqp.contains_active_advanced_search_options(), False
        )  # Clustering not an advanced search option
    ...
    def test_search_query_processor_as_query_params_exclude_facet_filters(self):
        for filter_name, is_facet in [
            ...
            self.assertEqual(
                f"{filter_name}:123" in sqp.as_query_params(exclude_facet_filters=True)["query_filter"], not is_facet
            )
            self.assertEqual(
                f"{filter_name}:123" in sqp.as_query_params(exclude_facet_filters=False)["query_filter"], True
            )
